...
bool CFMAutoPtr::blFirst=true;
HANDLE CFMAutoPtr::m_stHandler = NULL;
...
void CLogModule::OutputLog(char* info,...)
{
	...
	try
	{
		...
		//这个MYTRACE,如果写成MYTRACE(buffer);则可能会报错,因为buffer中可能会有%存在
		MYTRACE("%s",buffer);
#ifdef WRITE_LOG
...
{
	MYTRACE("CLocalConnection::CLocalConnection()");
	m_hWorkThread = NULL;
	m_ptrFuncCall = 0;
	m_ptrFuncErr = 0;

	m_hEventThreadClosed = ::CreateEvent(NULL,TRUE,FALSE,NULL);
	::ResetEvent(m_hEventThreadClosed);

	m_hEventThreadNeedExit = ::CreateEvent(NULL,TRUE,FALSE,NULL);
	::ResetEvent(m_hEventThreadNeedExit);
}

CLocalConnection::~CLocalConnection(void)
{
	::CloseHandle(m_hEventThreadClosed);
	::CloseHandle(m_hEventThreadNeedExit);
	MYTRACE("CLocalConnection::~CLocalConnection()");
}
...
void CLocalConnection::FinalRelease()
{
	...
	{
		::SetEvent(m_hEventThreadNeedExit);	

		if(WAIT_OBJECT_0 == ::WaitForSingleObject(m_hEventThreadClosed,2000))
		{
			OutputLog("检测到工作线程退出了");			
		}
		else
		{
			OutputLog("检测到工作线程未能正常退出.强制退出");
	...
	if(CFMAutoPtr::m_stHandler)
	{
		::CloseHandle(CFMAutoPtr::m_stHandler);
	}
}
...
DWORD WINAPI WorkingThread(LPVOID para)
{
	...
	::ResetEvent(pObj->m_hEventThreadClosed);
	::ResetEvent(pObj->m_hEventThreadNeedExit);

	static int idx = 0;
	while(WAIT_OBJECT_0 != ::WaitForSingleObject(pObj->m_hEventThreadNeedExit,SLEEP_TIME))
	{
	...
	::SetEvent(pObj->m_hEventThreadClosed);
	return 0;
}
//创建监听
//参数:创建的监听名称,接收调用消息的窗体
//返回值:0:成功  1:创建工作线程失败 2:这个名字已被使用. 3:异常
int CLocalConnection::Connect(char* szName)
{
	OutputLog("Connect.Name=%s",szName);
...
{
	OutputLog("LocalConnection.Close(%s)",szName);
	CString sz1;
	sz1.Format("%s",szName);
...
int CLocalConnection::ICheckLCName(CString szLocConnName,int regType)
{
	...
	int rtv = 0;
	HANDLE hCheckBreak = ::CreateEvent(NULL,TRUE,FALSE,NULL);
	::ResetEvent(hCheckBreak);

	while(WAIT_OBJECT_0 != ::WaitForSingleObject(hCheckBreak,SLEEP_TIME))
	{
	...
	::CloseHandle(hCheckBreak);
	MYTRACE("ICheckLCName.Rtv=%d",rtv);
	return rtv;
}
...
HANDLE CLocalConnection::ICreateFileMap(void)
{
	...
	else
	{
		...
		MYTRACE("创建文件映射对象失败:ErrCode:[%d]",errCode);
		//创建失败了.
...
bool CLocalConnection::ICheckRegistedName(byte* pBuff, CString szLocName)
{
	...
	while(len > 0)
	{
		//MYTRACE("LocName=[%s],Memory=[%s]",szLocName,szBuffer);
...
int CLocalConnection::IParseData(byte* pBuff)
{
	...
	pRd->m_szLCName.Format("%s",szName);
	...
	while((pt-pBuff - 16) < msgLen)
	{
		...
		*/
		...
		else if(pt[0] == 0x06)
		{
			...
			vt_struct vs;
			vs.vt = vt_string;
			vs.data.m_string = new char[strlen(szt)+1];
			::ZeroMemory(vs.data.m_string,strlen(szt)+1);
			strcpy(vs.data.m_string,szt);
			pRd->m_lstParas.push_back(vs);
		}
		else if(pt[0] == 0x0C)
		{
...
LRESULT CLocalConnection::OnFuncCallMsg(char *szName,CReceiveData *pd)
{
	...
	for(itr=pd->m_lstParas.begin();itr!=pd->m_lstParas.end();itr++)
	{
		...
		{
		case vt_int:
			MYTRACE("接收到的参数(数据类型:int):%d",(*itr).data.m_int);
			break;
		case vt_double:
			MYTRACE("接收到的参数(数据类型:double):%f",(*itr).data.m_double);
			break;
		case vt_string:
			MYTRACE("接收到的参数(数据类型:string):%s",(*itr).data.m_string);
			break;
		case vt_buffer:
			MYTRACE("接收到的参数(数据类型:ByteArray):长度为 %d",(*itr).data.m_buff.m_buffLen);
			break;
		default:
			MYTRACE("未知的参数类型");
			break;
	...
	typedef void (__stdcall *TFUNC)(int flag,char *lcName,char *methodName,void *ptrPara1,void *ptrPara2,void *ptrPara3);
	if(this->m_ptrFuncCall != 0)
	{
		int size = pd->m_lstParas.size();

		MYTRACE("现在执行回调函数:参数个数为:%d,LCName=%s,MethodName=%s",size,pd->m_szLCName.GetBuffer(),pd->m_szMethodName.GetBuffer());
		if(size == 1)
		{
			CAutoPtr<byte>p1(pd->getParam(0));
			
			TFUNC func = (TFUNC)m_ptrFuncCall;
			(*func)(1,pd->m_szLCName.GetBuffer(),pd->m_szMethodName.GetBuffer(),p1,NULL,NULL);
		}
		else if(size == 2)
		{
			CAutoPtr<byte>p1(pd->getParam(0));
			CAutoPtr<byte>p2(pd->getParam(1));
			
			TFUNC func = (TFUNC)m_ptrFuncCall;
			(*func)(2,pd->m_szLCName.GetBuffer(),pd->m_szMethodName.GetBuffer(),p1,p2,NULL);
		}
		else if(size == 3)
		{
			CAutoPtr<byte>p1(pd->getParam(0));
			CAutoPtr<byte>p2(pd->getParam(1));
			CAutoPtr<byte>p3(pd->getParam(2));

			TFUNC func = (TFUNC)m_ptrFuncCall;
			(*func)(3,pd->m_szLCName.GetBuffer(),pd->m_szMethodName.GetBuffer(),p1,p2,p3);
		}
		else
		{
			return 1;
	...
	return 0;
	/*
	//后面的代码保留.具体到对某个lc的某个method的指针设定
	//增加对回调的支持
...
int CLocalConnection::ISendData(byte *pBuff,CSendData &sd)
{
	...
	{		
		...
		{
		case vt_string:
			OutputLog("要传送的参数类型:vt_string.value=[%s]",(*itr).data.m_string);
			stream.WriteString(pt,(*itr).data.m_string,len);
			pt+=len;
			dwMsgLen+=len;
			break;
		case vt_buffer:
			OutputLog("要传送的参数类型:vt_buffer");
			OutputLog("参数数据长度:%d",(*itr).data.m_buff.m_buffLen);
			OutputLog("参数数据:");
			OutputLogHex((byte*)(*itr).data.m_buff.m_buff,(*itr).data.m_buff.m_buffLen);
			OutputLog("写数据之前:");
			OutputLogHex(pBuff,512);
			pt[0] = 0x0C;
			pt++;
			...
			stream.WriteInt29(pt,(*itr).data.m_buff.m_buffLen*2+1,len);
			pt+=len;
			dwMsgLen+=len;

			memcpy(pt,(*itr).data.m_buff.m_buff,(*itr).data.m_buff.m_buffLen);
			pt += (*itr).data.m_buff.m_buffLen;
			dwMsgLen += (*itr).data.m_buff.m_buffLen;
			OutputLog("写数据之后:");
			OutputLogHex(pBuff,512);
			break;
		default:
			OutputLog("要传送的参数:vt_unknown");
			break;
		}
...
int CLocalConnection::ICheckAndSendData(CSendData& sd)
{
	...
	::CloseHandle(hCheckBreak);
	OutputLog("ICheckAndSendData.RTV=%d",rtv);
	return rtv;
}
...
//参数是两个函数指针,
int CLocalConnection::FunConfig(int funcCall, int errCall)
{
	m_ptrFuncCall = funcCall;
	m_ptrFuncErr = errCall;
	return 0;
}
