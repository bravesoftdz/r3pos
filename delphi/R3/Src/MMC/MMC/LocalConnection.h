...
public:
	...
	~CFMAutoPtr()
	{
		...
		if(!m_blFirst)
		{
			//MYTRACE("FM.CloseHandler");
			::CloseHandle(m_lpHandle);
		}
	}
	bool m_blFirst;
	static bool blFirst;
	static HANDLE	m_stHandler;
	HANDLE m_lpHandle;
};
...
public:
	...
	//向列表插入一条记录
	inline void ListLC_Add(char *pszName)
	{
		m_csListLc.Lock();
		LCINFO lc(pszName);
	...
	int FunConfig(int funcCall, int errCall);

	int m_ptrFuncCall;
	int m_ptrFuncErr;

	//在工作线程退出的时候触发
	HANDLE m_hEventThreadClosed;
	//现在需要退出工作线程
	HANDLE	m_hEventThreadNeedExit;
};
