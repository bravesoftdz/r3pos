...
begin
  ...
  try
    try
      MsgFactory:=TMsgSyncFactory.Create;
      MsgFactory.dbResoler := dbResoler;
      MsgFactory.CallSyncData(GPlugIn,StrPas(Params));
      if not MsgFactory.HasError then //运行正常：
        result:=0
      else
        GLastError:=MsgFactory.ErrorMsg;
    finally
      MsgFactory.Free;
    end;
  except
