...
begin
  try
    ...
    try
      ...
      rs.SQL.Text:='select A.CUST_CODE,A.COM_ID from RM_CUST A,CA_SHOP_INFO B where A.LICENSE_CODE=B.LICENSE_CODE and B.TENANT_ID='+ParamList.ParambyName('TENANT_ID').asString+' and B.SHOP_ID='''+ParamList.ParambyName('SHOP_ID').asString+''' ';
      OpenData(GPlugIn, rs);
      ParamList.ParamByName('rimuid').AsString := rs.Fields[0].AsString;
      ParamList.ParamByName('rimpwd').AsString := rs.Fields[0].AsString;
