unit uGodsDzCaFactory;

interface
uses
  Windows, Messages, SysUtils, Classes,InvokeRegistry, SOAPHTTPClient, Types,
  XSBuiltIns,Des,WinInet, xmldom, XMLIntf,msxmldom, XMLDoc, MSHTML,ComObj,
  ActiveX,msxml,ZDataSet,DB,ZBase,Variants,ZLogFile,uBaseSyncFactory,Forms,
  uRimSyncFactory;
...
type
  ...
  private
    ...
    procedure doAfterExecute(const MethodName: string; SOAPResponse: TStream);
  ...
  protected
    ...
    procedure AddLogMsg(InfoType: integer; Msg: string); //添加本是下载的LogList
    procedure WriteToReportLogFile; //将日志写入REPORT20110825.log日志文件
  public
    auto:boolean;
    constructor Create;
    destructor Destroy;override;
    ...
    //2011.08.24 Add 提交数据UpdateBatch
    ...
    function  ExecSQL(SQL:string;var iRet:integer; Params: TftParamList=nil):integer; //执行SQL
    function  UpdateBatch(DataSet:TZQuery; AClassName:String; Params:TftParamList):Boolean; //提交数据
    ...
    //插件接口
    property PlugIntf: IPlugIn read GetPlugIntf;    
    //调用访问数据对象
...
uses EncDec,ZLibExGZ, encddecd,IniFiles,ObjCommon,
     CaTenantService,CaProductService,CaServiceLineService,RspDownloadService,PubMemberService;
...
begin
  ...
  if node.text<>'0000' then
    ...
    on E:Exception do
      begin
        if doc<>nil then
           AddLogMsg(0,E.Message+' XML='+doc.xml)
        else
           AddLogMsg(0,E.Message+' XML=无');
        Raise;
      end;
  end;
end;
...
begin
  try
    ...
    begin
      if doc<>nil then
         AddLogMsg(0,'认证<企业登录>失败xml='+doc.xml+';原因:'+E.Message)
      else
         AddLogMsg(0,'认证<企业登录>失败xml=无;原因:'+E.Message);
      Raise;
    end;
  end;
end;
...
begin
  try
    ...
    begin
      if doc<>nil then
         AddLogMsg(0,'认证<企业注册>失败xml='+doc.xml+';原因:'+E.Message)
      else
         AddLogMsg(0,'认证<企业注册>失败xml=无;原因:'+E.Message);
      Raise;
    end;
  end;
end;
...
try
  ...
  if doc<>nil then
     AddLogMsg(0,'读取<供应链资料>xml='+doc.xml)
  else
     AddLogMsg(0,'读取<供应链资料>xml=无');
  Raise;
end;
end;
...
try
  ...
  if doc<>nil then
     AddLogMsg(0,'读取<供应链列表>xml='+doc.xml)
  else
     AddLogMsg(0,'读取<供应链列表>xml=无');
  Raise;
end;
end;
...
begin
  ...
  LogList.Clear; //清空LogList
  try
    AutoCoLogo;
    //下载企业关系...
    downloadRelations(TENANT_ID,flag);
    //下载供应链...
    downloadServiceLines(TENANT_ID,flag);
    //下载企业资料...
    downloadTenants(TENANT_ID,flag);
    if TenantType <> 3 then //零售商不需要下载
    begin
      //下载商品分类...
      downloadSort(TENANT_ID,flag);
      //下载计量单位...
      downloadUnit(TENANT_ID,flag);
      //下载商品资料...
      downloadGoods(TENANT_ID,flag);
      //下载供应链商品...
      downloadDeployGoods(TENANT_ID,flag);
      //下载条码信息...
      downloadBarcode(TENANT_ID,flag);
    end;
    //下载商盟信息...
    downloadUnion(TENANT_ID);
    SetSynTimeStamp('#',TimeStamp,'#');
    //下载功能模块...
    downloadCaModule(TENANT_ID,flag);
    SetSynTimeStamp('#',TimeStamp,'#');
  finally
    //写入日志文件
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(1,'开始下载<供应关系>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<供应关系>打开时长:'+inttostr(GetTicket));
    SetTicket;
    ...
    try
      ...
      try
        ...
        UpdateBatch(rs,'TSyncSingleTable',Params); 
        SetSynTimeStamp('CA_RELATIONS',timeStamp,'#');
      finally
        Params.free;
      end;
      AddLogMsg(1,'保存<供应关系>执行时长:'+inttostr(GetTicket)+' 记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<供应关系>xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<供应关系>xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(1,'开始下载<供应链>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<供应链>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    try
      rs.Close;
      ...
      AddLogMsg(1,'保存<供应链>执行时长:'+inttostr(GetTicket)+' 记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<供应链>xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<供应链>xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(1,'开始下载<企业资料>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<企业资料>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    try
      rs.Close;
      ...
      AddLogMsg(1,'保存<企业资料>打开时长:'+inttostr(GetTicket)+'  记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<企业资料>xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<企业资料>xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(0,'开始下载<商品分类>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<商品分类>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    try
      rs.Close;
      ...
      AddLogMsg(1,'保存<商品分类>执行时长:'+inttostr(GetTicket)+' 记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<商品分类>xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<商品分类>xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(1,'开始下载<商品资料>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<商品资料>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    try
      rs.Close;
      ...
      AddLogMsg(1,'保存<商品资料>执行时长:'+inttostr(GetTicket)+' 记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<商品资料>xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<商品资料>xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(1,'开始下载<商品单位>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<计量单位>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    try
      rs.Close;
      ...
      AddLogMsg(1,'保存<计量单位>执行时长:'+inttostr(GetTicket)+' 记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<商品单位>失败xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<商品单位>失败xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(1,'开始下载<供应链商品>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<供应链商品>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    try
      rs.Close;
      ...
      AddLogMsg(1,'保存<供应链商品>执行时长:'+inttostr(GetTicket)+' 记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(1,'开始下载<商品条码>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<商品条码>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    try
      rs.Close;
      ...
      try
        ...
        Params.ParamByName('SYN_TIME_STAMP').Value := TimeStamp;
        Params.ParamByName('TIME_STAMP_NOCHG').AsInteger := 1;
        UpdateBatch(rs,'TSyncPubBarcode',Params);
        SetSynTimeStamp('PUB_BARCODE',timeStamp,'#');
      finally
        Params.free;
      end;
      AddLogMsg(1,'保存<下载条码>执行时长:'+inttostr(GetTicket)+' 记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<商品条码>xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<商品条码>xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
try
  ...
  AddLogMsg(1,'开始下载<商盟资料>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<商盟资料>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    ...
    try
           ...
           while IndexResp<>nil do
             begin
               ...
               if indexResp.nodeName='pubPricegrade' then
                  begin
                     prc.Append;
                     prc.FieldByName('TENANT_ID').AsInteger := TENANT_ID;
      ...
      try
        ...
        try
          ...
          UpdateBatch(idx,'TSyncSingleTable',Params);
          ...
          UpdateBatch(prc,'TSyncSingleTable',Params);
      ...
      AddLogMsg(1,'保存<商盟资料>打开时长:'+inttostr(GetTicket)+'  记录数:'+inttostr(rs.RecordCount));
    finally
      prc.Free;
      idx.Free;
      rs.Free;
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<商盟资料>xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<商盟资料>xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
try
  ...
  FindNode(doc,'header\pub').appendChild(Node);
  AddLogMsg(1,'开始下载<功能模块>上次同步时间:'+Node.text+' 本次同步时间:'+inttostr(TimeStamp));
  ...
  begin
    ...
    CheckRecAck(doc);
    Node := FindNode(doc,'body');
    AddLogMsg(1,'下载<功能模块>打开时长:'+inttostr(GetTicket));
    SetTicket;
    rs := TZQuery.Create(nil);
    try
      rs.Close;
      ...
      try
        ...
        Params.ParamByName('SYN_TIME_STAMP').Value := TimeStamp;
        Params.ParamByName('TIME_STAMP_NOCHG').AsInteger := 1;
        UpdateBatch(rs,'TSyncCaModule',Params);
        SetSynTimeStamp('CA_MODULE',timeStamp,ProductId);
      finally
        Params.free;
      end;
      AddLogMsg(1,'保存<功能模块>执行时长:'+inttostr(GetTicket)+' 记录数:'+inttostr(rs.RecordCount));
    finally
      rs.Free;
    end;
except
  ...
  begin
    if doc<>nil then
       AddLogMsg(0,'下载<功能模块>失败xml='+doc.xml+';原因:'+E.Message)
    else
       AddLogMsg(0,'下载<功能模块>失败xml=无;原因:'+E.Message);
    Raise;
  end;
end;
end;
...
procedure TCaFactory.SetTENANT_ID(const Value: integer);
begin
  FTENANT_ID := Value;
end;

procedure TCaFactory.SetDBFactory(const Value: TRimSyncFactory);
begin
  FDBFactory := Value;
  dbResoler:=FDBFactory.dbResoler;
end;

function TCaFactory.GetPlugIntf: IPlugIn;
begin
  result:=DBFactory.PlugIntf;
end;


function TCaFactory.ExecSQL(SQL: string; var iRet: integer; Params: TftParamList): integer;
begin
  result:=DBFactory.ExecSQL(Pchar(SQL),iRet,Params); 
end;

function TCaFactory.Open(DataSet: TDataSet; Params: TftParamList): Boolean;
begin
  result:=DBFactory.Open(DataSet,Params); 
end;

function TCaFactory.UpdateBatch(DataSet: TZQuery; AClassName: String; Params: TftParamList): Boolean;
var
  iRet: integer;
begin
  iRet:=DBFactory.UpdateBatch(DataSet.Data,PChar(AClassName),dbResoler,Params);
  if iRet<>0 then
    Raise Exception.Create(DBFactory.GetLastError);
end;

function TCaFactory.BeginTrans(TimeOut: integer): Boolean;
begin
  result:=DBFactory.BeginTrans(TimeOut);
end;

function TCaFactory.CommitTrans: Boolean;
begin
  result:=DBFactory.CommitTrans;
end;

function TCaFactory.RollbackTrans: Boolean;
begin
  result:=DBFactory.RollbackTrans;
end;

procedure TCaFactory.SetdbResoler(const Value: Integer);
begin
  FdbResoler := Value;
end;

{InfoType: 0表示错误日志，同时写入LogFile}
procedure TCaFactory.AddLogMsg(InfoType: integer; Msg: string);
var
  InfoMsg: string;
begin
  InfoMsg:='<'+TBaseSyncFactory.GetTickTime+'> '+Msg;
  LogList.Add(InfoMsg);
...
procedure TCaFactory.WriteToReportLogFile;
var
  i: integer;
  LogFile: string;
...
begin
  try
    FBegTickCount:=GetTickCount-FBegTickCount; //计算运行时间
    ...
    if FileExists(LogFile) then LogFileList.LoadFromFile(LogFile);

    LogList.Add('〖---- <Rsp调度> 【下载<'+InttoStr(TENANT_ID)+'>基础档案】 <'+FBeginTime+'>开始 ----〗');
    for i:=0 to LogList.Count-1 do
    begin
      LogFileList.Add(LogList.Strings[i]);
    end;                                                               
    LogList.Add('〖---- <Rsp调度> 【下载完毕】 <运行：'+FormatFloat('#0.00',FBegTickCount/1000)+'秒> ----〗');    
