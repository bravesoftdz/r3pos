...
function SendDayReck(PlugIntf:IPlugIn; TENANT_ID,SHOP_ID,ORGAN_ID,CustID,LICENSE_CODE,ReckDate:string; iDbType: integer):boolean;
var
  RunTimes,        //循环次数
...
begin
  ...
  case iDbType of
   1:
    begin
      Session:='';
      vReckDate:='trim(char(A.CREA_DATE))';
    end;
   4:
    begin
      Session:='session.';
      vReckDate:='cast(A.CREA_DATE as varchar(8))';
      Str:=
        'DECLARE GLOBAL TEMPORARY TABLE session.INF_RECKDAY( '+
             ...
             ' CO_NUM VARCHAR(50) NOT NULL, '+   //单据号
  ...
  //第一步: 大于时间戳的台帐插入临时表:
  vReckDateCnd:='';
  if ReckDate<>'' then vReckDateCnd:=' or (A.CREA_DATE='+ReckDate+')';
  CndTab:='select A.TENANT_ID,A.SHOP_ID,C.CREA_DATE from RCK_GOODS_DAYS A,VIW_GOODSINFO B where A.TENANT_ID=B.TENANT_ID and A.GODS_ID=B.GODS_ID and '+
          ...
          ' and ((A.TIME_STAMP>'+MaxStamp+')'+vReckDateCnd+') ';  //

  Str:='insert into '+Session+'INF_RECKDAY(TENANT_ID,SHOP_ID,COM_ID,CUST_ID,ITEM_ID,GODS_ID,RECK_DAY,QTY_ORD,AMT,TIME_STAMP) '+
     'select A.TENANT_ID,A.SHOP_ID,'''+Short_ID+''' as SHORT_SHOP_ID,'''+ORGAN_ID+''' as COM_ID,'''+CustID+''' as CUST_ID,B.SECOND_ID,A.GODS_ID,'+vReckDate+' as RECK_DAY,'+
     ' (case when '+GetDefaultUnitCalc+'<>0 then A.SALE_AMT/('+GetDefaultUnitCalc+') else A.SALE_AMT end) as SALE_AMT,A.SALE_RTL,A.TIME_STAMP,('+vReckDate+' || ''_'' || '''+CustID+''' ||''_'' || '''+Short_ID+''') as CO_NUM '+
     ' from RCK_GOODS_DAYS A,VIW_GOODSINFO B,('+CndTab+')C '+
     ' where A.TENANT_ID=B.TENANT_ID and A.TENANT_ID=C.TENANT_ID and A.GODS_ID=B.GODS_ID and A.CREA_DATE=C.CREA_DATE and A.TENANT_ID='+TENANT_ID+
     ' and A.SHOP_ID='''+SHOP_ID+''' and B.RELATION_ID='+InttoStr(NT_RELATION_ID);
