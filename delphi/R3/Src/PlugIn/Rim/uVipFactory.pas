...
begin
  ...
  try
    ...
    if rs.IsEmpty then
    begin
      EndLogShopReport(ShopInfo+'在RIM中没找到...',False,False);
    ...
    //写日志
    if DCount+DErrCount+UCount+UErrCount=0 then
      EndLogShopReport(ShopInfo+'：无', true, true)
    else
    begin
      ...
      begin
        if UCount>0 then
          UpMsg:='上传:'+InttoStr(UCount);
        if UErrCount>0 then
        begin
          if UCount>0 then UpMsg:=UpMsg+';错误：'+InttoStr(UErrCount)+']' else UpMsg:='上传错误:'+InttoStr(UErrCount)+'';
        end;
      end;
      EndLogShopReport(ShopInfo+'['+DownMsg+UpMsg+']', true, (DErrCount+UErrCount>0));
    end;
...
procedure TVipSyncFactory.WriteToReportLogFile(ReportName: string);
var
  i: integer;
