...
begin
  ...
  StrSql :=
    'select A.TENANT_ID,A.GODS_ID,A.GODS_CODE,A.GODS_NAME,A.BARCODE,A.SHOP_NAME,A.CALC_UNITS as UNIT_ID,'+
       'isnull(A.NEW_OUTPRICE,0) as NEW_OUTPRICE_1,'+
       'isnull(B.NEW_OUTPRICE,0) as ORG_OUTPRICE,'+
       'isnull(B.NEW_OUTPRICE1,0) as ORG_OUTPRICE1,'+
       'isnull(B.NEW_OUTPRICE2,0) as ORG_OUTPRICE2,'+
       'A.SMALLTO_CALC,'+
       'A.BIGTO_CALC,'+
       '(case when cast(A.NEW_OUTPRICE*100.00 as decimal(18,3))<>0 then isnull(B.NEW_OUTPRICE,0)*1.00/A.NEW_OUTPRICE*100.00 else 0.0 end)as OUTPRICE_A_RATE,'+
       'isnull(B.NEW_OUTPRICE,0) as NEW_OUTPRICE,'+
       '(case when cast(A.NEW_OUTPRICE*100.00 as decimal(18,3))<>0 then isnull(B.NEW_OUTPRICE,0)*1.00/A.NEW_OUTPRICE*100.00 else 0.0 end)as NEW_OUTPRICE_RATE,'+
       '''#'' as PRICE_ID,A.SHOP_ID,isnull(B.PRICE_METHOD,''1'') as PRICE_METHOD,B.NEW_OUTPRICE1,B.NEW_OUTPRICE2 '+
    'from '+
     ' (select A1.*,C1.SHOP_ID,C1.SHOP_NAME,C1.REGION_ID from VIW_GOODSINFO A1 '+
     ' inner join CA_SHOP_INFO C1 on A1.TENANT_ID=C1.TENANT_ID '+ StrWhere+') A '+
   ' left outer join PUB_GOODSPRICE B on A.TENANT_ID=B.TENANT_ID and A.GODS_ID=B.GODS_ID and A.SHOP_ID=B.SHOP_ID and A.PRICE_ID=B.PRICE_ID';
  Result := StrSql;
end;
