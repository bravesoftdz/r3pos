...
begin
  ...
  'from (select A1.*,C1.SHOP_ID,C1.SHOP_NAME,C1.REGION_ID from VIW_GOODSINFO A1 inner join CA_SHOP_INFO C1 on A1.TENANT_ID=C1.TENANT_ID '+ StrWhere+') A left outer join PUB_GOODSPRICE B on A.TENANT_ID=B.TENANT_ID and A.GODS_ID=B.GODS_ID and A.SHOP_ID=B.SHOP_ID and A.PRICE_ID=B.PRICE_ID '+
  ' ';
  Result := StrSql;
end;
