...
type
  TfrmCancelCard = class(TframeDialogForm)
    Btn_Save: TRzBitBtn;
    Btn_Close: TRzBitBtn;
    RzLabel1: TRzLabel;
...
procedure TfrmCancelCard.Btn_CloseClick(Sender: TObject);
begin
  inherited;
  close;
end;

procedure TfrmCancelCard.FormShow(Sender: TObject);
begin
  inherited;
  //if edtIC_INFO.CanFocus then edtIC_INFO.SetFocus;
end;

procedure TfrmCancelCard.Btn_SaveClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmCancelCard.edtIC_INFOKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure TfrmCancelCard.InitControl;
var Str_Sql:String;
    rs:TZQuery;
    AObj_:TRecord_;
begin
...
procedure TfrmCancelCard.SetCust_Id(const Value: String);
begin
  FCust_Id := Value;
end;

procedure TfrmCancelCard.FormCreate(Sender: TObject);
begin
  inherited;
  InitControl;
end;

class function TfrmCancelCard.SelectCard(Owner: TForm; CUSTID,
  UNION_ID: string;var C_N0,C_Name:String): boolean;
begin
  with TfrmCancelCard.Create(Owner) do
    begin
      try
        Cust_Id := CUSTID;
        edtUNION_ID.ItemIndex := TdsItems.FindItems(edtUNION_ID.Properties.Items,'UNION_ID',UNION_ID);
...
procedure TfrmCancelCard.edtUNION_IDPropertiesChange(Sender: TObject);
var UnionId:String;
begin
  inherited;
