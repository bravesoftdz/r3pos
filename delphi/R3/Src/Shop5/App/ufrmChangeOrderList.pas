...
begin
  ...
  if fndSTATUS.ItemIndex > 0 then
     begin
       ...
       3: w := w + ' and A.PRINT_TIMES is null ';
       end;
     end;
  if id<>'' then
  ...
  result := 'select A.CHANGE_ID,A.CHANGE_DATE,A.GLIDE_NO,A.CHANGE_TYPE,A.REMARK,A.DUTY_USER,A.CHANGE_CODE,A.TENANT_ID,A.DEPT_ID,A.SHOP_ID,A.CREA_DATE,A.CREA_USER,CHANGE_AMT as AMOUNT,CHANGE_MNY as AMONEY '+
            'from STO_CHANGEORDER A '+w+ShopGlobal.GetDataRight('A.SHOP_ID',1);
...
begin
  ...
  if TfrmChangeOrder(CurOrder).cdsHeader.FieldByName('CREA_USER').AsString <> Global.UserID then
    begin
      if CodeId = '1' then
...
begin
  ...
  result :=
   ...
   'where A.TENANT_ID=B.TENANT_ID and A.CHANGE_ID=B.CHANGE_ID and A.TENANT_ID='+tenantid+' and A.CHANGE_ID='''+id+''' '+order1+' ) jb '+
   ...
   'left outer join VIW_USERS m on jm.TENANT_ID=m.TENANT_ID and jm.LOCUS_USER=m.USER_ID ) j '+order2;
