...
begin
  ...
  begin
    result :=
     ...
     'case when isnull(jc.AMOUNT,0)>0 then isnull(jc.AMOUNT,0)*1.00/(case when B.UNIT_ID=B.SMALL_UNITS then B.SMALLTO_CALC when B.UNIT_ID=B.BIG_UNITS then B.BIGTO_CALC else 1 end) else 0 end as LOSS_AMOUNT, '+
     'case when isnull(jc.AMOUNT,0)<0 then - isnull(jc.AMOUNT,0)*1.00/(case when B.UNIT_ID=B.SMALL_UNITS then B.SMALLTO_CALC when B.UNIT_ID=B.BIG_UNITS then B.BIGTO_CALC else 1 end) else 0 end as PROFIT_AMOUNT, '+
