...
begin
  ...
  //2012.05.16修改增加数据权限
  StrText :=
    'select A.*,B.UNION_ID,B.IC_CARDNO,B.ACCU_INTEGRAL,B.RULE_INTEGRAL,B.INTEGRAL,B.BALANCE from '+
      '(select 0 as selflag,TENANT_ID,CLIENT_ID,CLIENT_TYPE,CLIENT_CODE,LICENSE_CODE,CLIENT_NAME,CLIENT_SPELL,SORT_ID,REGION_ID,SETTLE_CODE,'+
       ...
       'PRICE_ID,SHOP_ID,''#'' as UNION_ID from PUB_CLIENTINFO '+
      ' where COMM not in (''02'',''12'') and CLIENT_TYPE=''2'' and TENANT_ID='+IntToStr(Global.TENANT_ID)+Str_Where+ShopGlobal.GetDataRight('SHOP_ID',1)+') A '+
     ' left join PUB_IC_INFO B on A.CLIENT_ID=B.CLIENT_ID and A.TENANT_ID=B.TENANT_ID and A.UNION_ID=B.UNION_ID '+
     ' order by A.CLIENT_CODE ';

  {if ShopGlobal.GetChkRight('33300001',7) then
     begin
     end
  else
     begin
