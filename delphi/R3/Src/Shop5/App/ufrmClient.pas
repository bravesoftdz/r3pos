...
begin
  ...
  if ShopGlobal.GetChkRight('33300001',7) then
     begin
      ...
      +Str_Where+ShopGlobal.GetDataRight('SHOP_ID',1)+') A left join PUB_IC_INFO B on A.CLIENT_ID=B.CLIENT_ID and A.TENANT_ID=B.TENANT_ID and A.UNION_ID=B.UNION_ID order by A.CLIENT_CODE ';
     end
  else
     begin
      StrText := 'select A.*,B.UNION_ID,B.IC_CARDNO,B.ACCU_INTEGRAL,B.RULE_INTEGRAL,B.INTEGRAL,B.BALANCE from ('+
