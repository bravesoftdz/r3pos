...
begin
  ...
  SQLData:= //'VIW_SALESDATA';
    ...
    ' from SAL_SALESORDER A,SAL_SALESDATA B '+
    ' where A.TENANT_ID=B.TENANT_ID and A.SALES_ID=B.SALES_ID and A.SALES_TYPE in (1,3,4) and A.COMM not in (''02'',''12''))';
...
begin
  ...
  //客户群体所属行政区域|客户等级\客户分类:
  if (fndP2_CUST_VALUE.AsString<>'') then   //and ((fndP2_CUST_VALUE.AsString<>'#')and(fndP2_CUST_VALUE.Text<>'无')) 
  begin
