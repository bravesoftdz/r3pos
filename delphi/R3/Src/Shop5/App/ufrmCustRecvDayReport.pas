...
type
  TfrmCustRecvDayReport = class(TframeBaseReport)
    ...
    fndP5_POS: TcxRadioButton;
    fndP5_CardID: TcxRadioButton;
    fndP5_RECV: TcxRadioButton;
...
procedure TfrmCustRecvDayReport.FormCreate(Sender: TObject);
begin
  inherited;
  IsOnDblClick:=False;
...
function TfrmCustRecvDayReport.GetGroupSQL(chk:boolean=true): string;
var
...
function TfrmCustRecvDayReport.GetRowType: integer;
begin
  result :=0;  // DBGridEh.DataSource.DataSet.FieldbyName('grp0').AsInteger;
end;

procedure TfrmCustRecvDayReport.actFindExecute(Sender: TObject);
var strSql: string;
begin
...
function TfrmCustRecvDayReport.GetShopSQL(chk:boolean=true): string;
var
  strSql,RecvData,strWhere: string;
begin
...
function TfrmCustRecvDayReport.GetSortSQL(chk:boolean=true): string;
var
  strSql,RecvData,strWhere: string;
begin
...
function TfrmCustRecvDayReport.GetRecvUserSQL(chk:boolean=true): string;
var
  strSql,RecvData,strWhere: string;
begin
...
procedure TfrmCustRecvDayReport.DBGridEh1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmCustRecvDayReport.DBGridEh2DblClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmCustRecvDayReport.DBGridEh3DblClick(Sender: TObject);
var
  CurDate: string;
begin
  inherited;
...
procedure TfrmCustRecvDayReport.DBGridEh4DblClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmCustRecvDayReport.FormDestroy(Sender: TObject);
begin
  TDbGridEhSort.FreeForm(self);
  inherited;
end;

procedure TfrmCustRecvDayReport.InitGrid;
...
procedure TfrmCustRecvDayReport.PrintBefore;
var
  ReStr: string;
  Title: TStringList;
begin
...
function TfrmCustRecvDayReport.AddReportReport(TitleList: TStringList; PageNo: string): string;
var
...
function TfrmCustRecvDayReport.GetShopIDCnd(ShopID: TzrComboBoxList; FieldName: string): string;
begin
  result:='';
...
function TfrmCustRecvDayReport.GetDateCnd(BegDate, EndDate: TcxDateEdit; FieldName: string): string;
begin
  result:='';
...
function TfrmCustRecvDayReport.GetShopGroupCnd(SHOP_TYPE: TcxComboBox; TYPE_VALUE: TzrComboBoxList; AliasName: string): string;
var
  AName: string;
begin
  result:='';
...
function TfrmCustRecvDayReport.GetRckMaxDate(EndDate: integer): integer;
var
  rs:TZQuery;
begin
...
function TfrmCustRecvDayReport.GetRcevBaseData(fndBegDate, fndEndDate: TcxDateEdit): string;
var
...
begin
  ...
  begin
    ...
    Str:='select TENANT_ID,SHOP_ID,RECV_DATE,CREA_USER,CHK_DATE '+
       ',(case when ACC_DATE<>'+vBegDate+' then RECV_MNY else 0 end) as ORG_ALL_MNY '+   //收款合计:往日
       ',(case when ACC_DATE='+vBegDate+'  then RECV_MNY else 0 end) as NEW_ALL_MNY '+   //收款合计:本期
  ...
  begin
    ...
    Str:='select TENANT_ID,SHOP_ID,RECV_DATE,CREA_USER,CHK_DATE '+
      ',(case when (ACC_DATE<'+vBegDate+' or ACC_DATE>'+vEndDate+') then RECV_MNY else 0 end) as ORG_ALL_MNY '+   //收款合计:往日
      ',(case when (ACC_DATE>='+vBegDate+') and (ACC_DATE<='+vEndDate+') then RECV_MNY else 0 end) as NEW_ALL_MNY '+   //收款合计:本期
      ', RECV_MNY as ALL_MNY '+                                                         //收款小计
...
function TfrmCustRecvDayReport.GetGuideSQl(chk: boolean): string;
var
...
begin
  ...
  if fndP5_POS.Checked then
    strWhere:=strWhere+' and BILL_TYPE=''1'' '
  else if fndP5_CardID.Checked then
    strWhere:=strWhere+' and BILL_TYPE=''2'' '
  else if fndP5_RECV.Checked then
    strWhere:=strWhere+' and BILL_TYPE=''3'' ';

  //关联语句
  strSql:=
    'select jp.*,r.USER_NAME as USER_NAME from '+
    '(select jb.*,D.CLIENT_NAME as CUST_NAME from  '+
    '(select A.* from VIW_CUSTRECVALLDATA A,CA_SHOP_INFO B '+
    ' where A.TENANT_ID=B.TENANT_ID and A.SHOP_ID=B.SHOP_ID and A.TENANT_ID='+InttoStr(Global.TENANT_ID)+
    ' '+strWhere+') jb '+
    ' left outer join VIW_CUSTOMER D '+
    ' on jb.TENANT_ID=D.TENANT_ID and ((Jb.BILL_TYPE=''3'' and jb.CLIENT_ID=D.CLIENT_ID) or (jb.BILL_TYPE=''2'' and jb.CLIENT_ID=D.IC_CARDNO)))jp '+
    ' left outer join viw_users r on '+
    ' jp.TENANT_ID=r.TENANT_ID and jp.CREA_USER=r.USER_ID order by jp.CREA_USER ';
    
  Result := ParseSQL(Factor.iDbType,strSql);
end;

procedure TfrmCustRecvDayReport.DBGridEh1GetFooterParams(Sender: TObject;
...
procedure TfrmCustRecvDayReport.DBGridEh2GetFooterParams(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; AFont: TFont;
  var Background: TColor; var Alignment: TAlignment; State: TGridDrawState;
  var Text: String);
begin
...
procedure TfrmCustRecvDayReport.DBGridEh3GetFooterParams(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; AFont: TFont;
  var Background: TColor; var Alignment: TAlignment; State: TGridDrawState;
  var Text: String);
begin
...
procedure TfrmCustRecvDayReport.DBGridEh4GetFooterParams(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; AFont: TFont;
  var Background: TColor; var Alignment: TAlignment; State: TGridDrawState;
  var Text: String);
begin
...
procedure TfrmCustRecvDayReport.DBGridEh5GetFooterParams(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; AFont: TFont;
  var Background: TColor; var Alignment: TAlignment; State: TGridDrawState;
  var Text: String);
begin
