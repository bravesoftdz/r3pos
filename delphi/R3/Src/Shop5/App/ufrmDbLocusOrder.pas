...
begin
  ...
  if IsAudit then
     begin
       ...
       if cdsHeader.FieldByName('LOCUS_CHK_USER').AsString<>Global.UserID then Raise Exception.Create('只有审核人才能对当前调拨单执行弃审');
  ...
  try
    ...
    try
      ...
      if not IsAudit then
         Msg := Factor.ExecProc('TDbLocusNoAudit',Params)
      else
         Msg := Factor.ExecProc('TDbLocusNoUnAudit',Params) ;
    finally
       Params.free;
    end;
...
function TfrmDbLocusOrder.CalcCurr: real;
var
  amt:integer;
begin
