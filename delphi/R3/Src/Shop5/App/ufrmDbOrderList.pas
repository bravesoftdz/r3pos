...
begin
  ...
  if Copy(Global.SHOP_ID,Length(Global.SHOP_ID)-3,Length(Global.SHOP_ID)) <> '0001' then
     w := w +' and (A.CLIENT_ID='''+Global.SHOP_ID+''' or A.SHOP_ID='''+Global.SHOP_ID+''')';
  ...
  result := 'select A.TENANT_ID,A.SALES_ID,A.GLIDE_NO,A.SALES_DATE,A.SALES_TYPE,A.PLAN_DATE,A.REMARK,A.CLIENT_ID,A.CREA_USER,A.SHOP_ID,A.GUIDE_USER,A.CREA_DATE,A.SALE_AMT as AMOUNT,A.SALE_MNY as AMONEY, '+
            'case when A.LOCUS_STATUS = ''3'' then 3 else 1 end as LOCUS_STATUS_NAME1 from SAL_SALESORDER A '+w+' ';
  ...
  result := 'select jf.*,f.GUIDE_USER as STOCK_USER,case when f.LOCUS_STATUS = ''3'' then 3 else 1 end as LOCUS_STATUS_NAME2 from ('+result+') jf left outer join STK_STOCKORDER f on jf.TENANT_ID=f.TENANT_ID and jf.SALES_ID=f.STOCK_ID and jf.SALES_TYPE=f.STOCK_TYPE ';
  result := 'select jg.*,g.USER_NAME as STOCK_USER_TEXT from ('+result+') jg left outer join VIW_USERS g on jg.TENANT_ID=g.TENANT_ID and jg.STOCK_USER=g.USER_ID '+w1;
...
begin
  ...
  if CurOrder=nil then Raise Exception.Create('"调拔单"打开异常!');

  if TfrmdbOrder(CurOrder).cdsHeader.FieldByName('CREA_USER').AsString <> Global.UserID then
    begin
      if not ShopGlobal.GetChkRight('14100001',6) then
...
procedure TfrmDbOrderList.PrintAfter(Sender: TObject);
var Sql_Str:String;
begin
  if CurOrder<>nil then
...
procedure TfrmDbOrderList.PrintBefore(Sender: TObject);
var rs:TZQuery;
    Sql_Str,Info:String;
    i:Integer;
begin
