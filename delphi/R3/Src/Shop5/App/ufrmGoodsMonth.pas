...
begin
  ...
  StrSql :=
    ...
    'case when sum(isnull(BAL_AMT,0))=0 then 0 else sum(isnull(BAL_CST,0)-isnull(ADJ_CST,0))*1.000/sum(isnull(BAL_AMT,0)) end as BAL_PRICE,sum(isnull(ADJ_CST,0)) as ADJ_CST,'+
    'case when sum(isnull(BAL_AMT,0))=0 then 0 else sum(isnull(BAL_CST,0))*1.000/sum(isnull(BAL_AMT,0)) end as ADJ_PRICE,'+
