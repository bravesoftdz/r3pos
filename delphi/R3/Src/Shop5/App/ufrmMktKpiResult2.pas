...
type
  TfrmMktKpiResult2 = class(TframeToolForm)
...
procedure TfrmMktKpiResult2.FormCreate(Sender: TObject);
begin
  inherited;
  cdsKPI_ID.Close;
...
procedure TfrmMktKpiResult2.actFindExecute(Sender: TObject);
begin
  inherited;
  if not ShopGlobal.GetChkRight('100002164',1) then Raise Exception.Create('你没有查询'+Caption+'的权限,请和管理员联系.');
  Open('');
end;

procedure TfrmMktKpiResult2.actAuditExecute(Sender: TObject);
var
  Msg :string;
  Params:TftParamList;
begin
  inherited;
  if not ShopGlobal.GetChkRight('100002164',3) then Raise Exception.Create('你没有审核'+Caption+'的权限,请和管理员联系.');
  if IsAudit then
     begin
...
procedure TfrmMktKpiResult2.actPrintExecute(Sender: TObject);
begin
  inherited;
  if not ShopGlobal.GetChkRight('100002164',4) then Raise Exception.Create('你没有打印'+Caption+'的权限,请和管理员联系.');
  PrintView;
  PrintDBGridEh1.Print;
end;

procedure TfrmMktKpiResult2.actPreviewExecute(Sender: TObject);
begin
  inherited;
  if not ShopGlobal.GetChkRight('100002164',4) then Raise Exception.Create('你没有预览'+Caption+'的权限,请和管理员联系.');
  PrintView;
...
procedure TfrmMktKpiResult2.actSaveExecute(Sender: TObject);
var Stream: TMemoryStream;
begin
  inherited;
  if not ShopGlobal.GetChkRight('100002164',5) then Raise Exception.Create('你没有导出'+Caption+'的权限,请和管理员联系.');
...
procedure TfrmMktKpiResult2.actEditExecute(Sender: TObject);
begin
  inherited;
  if not ShopGlobal.GetChkRight('100002164',2) then Raise Exception.Create('你没有计算'+Caption+'的权限,请和管理员联系.');
...
procedure TfrmMktKpiResult2.GridDrawColumnCell(Sender: TObject;
...
function TfrmMktKpiResult2.EncodeSql(id: String): String;
var w,w1:string;
begin
  w := ' where A.TENANT_ID=:TENANT_ID and A.IDX_TYPE=''2'' and A.KPI_YEAR=:KPI_YEAR and A.COMM not in (''02'',''12'') ';
...
procedure TfrmMktKpiResult2.Open(Id: String);
var
  rs:TZQuery;
  sm:TMemoryStream;
begin
...
procedure TfrmMktKpiResult2.PrintView;
begin
...
procedure TfrmMktKpiResult2.fndCUST_TYPEPropertiesChange(Sender: TObject);
begin
...
procedure TfrmMktKpiResult2.FormDestroy(Sender: TObject);
begin
  inherited;
  CdsClient.Free;
end;

procedure TfrmMktKpiResult2.actInfoExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmMktKpiResult2.GridDblClick(Sender: TObject);
begin
  inherited;
  actInfo.OnExecute(nil);
end;

procedure TfrmMktKpiResult2.CdsKpiResultAfterScroll(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmMktKpiResult2.SetIsAudit(const Value: Boolean);
begin
  FIsAudit := Value;
  if Value then
     actAudit.Caption := '弃审'
...
procedure TfrmMktKpiResult2.FormShow(Sender: TObject);
begin
  inherited;
  fndCUST_TYPE.ItemIndex := 0;
end;

procedure TfrmMktKpiResult2.GridDrawFooterCell(Sender: TObject; DataCol,
