...
type
  TfrmMktMarketCost = class(TframeContractForm)
    RzLabel2: TRzLabel;
    RzLabel4: TRzLabel;
    Label3: TLabel;
...
procedure TfrmMktMarketCost.AuditOrder;
var
  Msg :string;
  Params:TftParamList;
begin
...
procedure TfrmMktMarketCost.CancelOrder;
begin
  inherited;
...
procedure TfrmMktMarketCost.DeleteOrder;
begin
  inherited;
  Saved := false;
...
procedure TfrmMktMarketCost.EditOrder;
begin
  inherited;
...
procedure TfrmMktMarketCost.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmMktMarketCost.NewOrder;
var rs:TZQuery;
begin
  inherited;
  Open('');
...
procedure TfrmMktMarketCost.Open(id: string);
var
  Params:TftParamList;
begin
  inherited;
...
procedure TfrmMktMarketCost.SaveOrder;
...
procedure TfrmMktMarketCost.edtKPI_IDEnter(Sender: TObject);
begin
  inherited;
  edtKPI_ID.Properties.ReadOnly := DBGridEh1.ReadOnly;
end;

procedure TfrmMktMarketCost.edtKPI_IDExit(Sender: TObject);
begin
  inherited;
  if not edtKPI_ID.DropListed then edtKPI_ID.Visible := False;
end;

procedure TfrmMktMarketCost.edtKPI_IDKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmMktMarketCost.FocusNextColumn;
var i:Integer;
begin
  i:=DbGridEh1.Col;
  Inc(i);
...
procedure TfrmMktMarketCost.InitRecord;
var tmp:TZQuery;
begin
...
procedure TfrmMktMarketCost.edtKPI_IDKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if Key=#13 then
...
procedure TfrmMktMarketCost.edtKPI_IDSaveValue(Sender: TObject);
...
procedure TfrmMktMarketCost.DBGridEh1KeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if Key=#13 then
...
procedure TfrmMktMarketCost.SetdbState(const Value: TDataSetState);
...
procedure TfrmMktMarketCost.DBGridEh1Columns1BeforeShowControl(
  Sender: TObject);
begin
  inherited;
...
function TfrmMktMarketCost.GetIsNull: boolean;
begin
  cdsDetail.First;
  while not cdsDetail.Eof do
    begin
...
procedure TfrmMktMarketCost.FormShow(Sender: TObject);
begin
  inherited;
  //
end;
...
procedure TfrmMktMarketCost.DeleteClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmMktMarketCost.DBGridEh1DrawFooterCell(Sender: TObject;
...
procedure TfrmMktMarketCost.edtCLIENT_IDPropertiesChange(Sender: TObject);
begin
  inherited;
  if trim(edtCLIENT_ID.Text)<>'' then TabSheet.Caption := edtCLIENT_ID.Text;
end;

procedure TfrmMktMarketCost.DBGridEh1DblClick(Sender: TObject);
begin
  inherited;
  N1Click(Sender);
end;

procedure TfrmMktMarketCost.N1Click(Sender: TObject);
begin
  inherited;
  if not cdsDetail.Active then Exit;
...
procedure TfrmMktMarketCost.SingleContractExtensionFrom(Id: String);
var a,b:TZQuery;
