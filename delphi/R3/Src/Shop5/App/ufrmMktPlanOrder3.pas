...
type
  TfrmMktPlanOrder3 = class(TframeContractForm)
    RzLabel2: TRzLabel;
    RzLabel4: TRzLabel;
    Label3: TLabel;
...
uses uGlobal, uCtrlUtil,uShopGlobal, uShopUtil, uFnUtil, uDsUtil, ufrmBasic, uXDictFactory,
     ufrmKpiIndexInfo, ufrmMktPlanOrderList3;
{$R *.dfm}

procedure TfrmMktPlanOrder3.AuditOrder;
var
  Msg :string;
  Params:TftParamList;
begin
...
//       if copy(cdsHeader.FieldByName('COMM').AsString,1,1)= '1' then Raise Exception.Create('已经同步的数据不能再审核');
  ...
  try
    ...
    try
      ...
      if not IsAudit then
         Msg := Factor.ExecProc('TMktPlanOrderAudit3',Params)
      else
         Msg := Factor.ExecProc('TMktPlanOrderUnAudit3',Params) ;
    finally
       Params.free;
    end;
...
procedure TfrmMktPlanOrder3.CancelOrder;
begin
  inherited;
...
procedure TfrmMktPlanOrder3.DeleteOrder;
begin
  inherited;
  Saved := false;
  ...
  Factor.BeginBatch;
  try
    Factor.AddBatch(cdsHeader,'TMktPlanOrder3');
    Factor.AddBatch(cdsDetail,'TMktPlanData3');
    Factor.CommitBatch;
    Saved := true;
  except
    Factor.CancelBatch;
...
procedure TfrmMktPlanOrder3.EditOrder;
begin
  inherited;
...
procedure TfrmMktPlanOrder3.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmMktPlanOrder3.NewOrder;
var rs:TZQuery;
begin
  inherited;
  Open('');
...
procedure TfrmMktPlanOrder3.Open(id: string);
var
  Params:TftParamList;
begin
  inherited;
  ...
  try
    ...
    Factor.BeginBatch;
    try
      Factor.AddBatch(cdsHeader,'TMktPlanOrder3',Params);
      Factor.AddBatch(cdsDetail,'TMktPlanData3',Params);
      Factor.OpenBatch;
    except
      Factor.CancelBatch;
      Raise;
...
procedure TfrmMktPlanOrder3.SaveOrder;
...
begin
  ...
  try
    ...
    if cdsDetail.IsEmpty then Raise Exception.Create('不能保存一张空采购合同单据...');
    ...
    AObj.FieldByName('PLAN_TYPE').AsString := '3';
...
procedure TfrmMktPlanOrder3.edtKPI_IDEnter(Sender: TObject);
begin
  inherited;
  edtKPI_ID.Properties.ReadOnly := DBGridEh1.ReadOnly;
end;

procedure TfrmMktPlanOrder3.edtKPI_IDExit(Sender: TObject);
begin
  inherited;
  if not edtKPI_ID.DropListed then edtKPI_ID.Visible := False;
end;

procedure TfrmMktPlanOrder3.edtKPI_IDKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmMktPlanOrder3.FocusNextColumn;
var i:Integer;
begin
  i:=DbGridEh1.Col;
  Inc(i);
...
procedure TfrmMktPlanOrder3.InitRecord;
var tmp:TZQuery;
begin
...
procedure TfrmMktPlanOrder3.edtKPI_IDKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if Key=#13 then
...
procedure TfrmMktPlanOrder3.edtKPI_IDSaveValue(Sender: TObject);
...
procedure TfrmMktPlanOrder3.DBGridEh1KeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if Key=#13 then
...
procedure TfrmMktPlanOrder3.SetdbState(const Value: TDataSetState);
...
procedure TfrmMktPlanOrder3.DBGridEh1Columns1BeforeShowControl(
  Sender: TObject);
begin
  inherited;
...
function TfrmMktPlanOrder3.GetIsNull: boolean;
begin
  cdsDetail.First;
  while not cdsDetail.Eof do
    begin
...
procedure TfrmMktPlanOrder3.FormShow(Sender: TObject);
begin
  inherited;
  //
end;

procedure TfrmMktPlanOrder3.edtKPI_YEARPropertiesChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmMktPlanOrder3.DBGridEh1DrawFooterCell(Sender: TObject;
...
procedure TfrmMktPlanOrder3.edtCLIENT_IDPropertiesChange(Sender: TObject);
begin
  inherited;
  if trim(edtCLIENT_ID.Text)<>'' then TabSheet.Caption := edtCLIENT_ID.Text;
end;

procedure TfrmMktPlanOrder3.DBGridEh1DblClick(Sender: TObject);
begin
  inherited;
  N1Click(Sender);
end;

procedure TfrmMktPlanOrder3.N1Click(Sender: TObject);
begin
  inherited;
  if not cdsDetail.Active then Exit;
...
procedure TfrmMktPlanOrder3.DBGridEh1DrawColumnCell(Sender: TObject;
...
procedure TfrmMktPlanOrder3.N3Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmMktPlanOrder3.SingleContractExtensionFrom(Id:String);
var a,b:TZQuery;
...
begin
  ...
  try
    ...
    Factor.BeginBatch;
    try
      Factor.AddBatch(a,'TMktPlanOrder3',Params_1);
      Factor.AddBatch(b,'TMktPlanData3',Params_1);
      Factor.OpenBatch;
