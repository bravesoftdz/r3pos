...
begin
  ...
  if edtPAYM_ID.ItemIndex < 0 then Raise Exception.Create('请选择缴款方式名称');
  if edtITEM_ID.AsString = '' then Raise Exception.Create('请选择收支科目名称');
  if edtRECV_DATE.EditValue = null then Raise Exception.Create('请选择缴款日期');
  ...
  cdsHeader.Post;
  if n=0 then Raise Exception.Create('没有缴款记录，不能保存...');
  Factor.BeginBatch;
  try
...
begin
  ...
  begin
    try
      i:=MessageBox(Handle,'缴款单有修改，是否保存修改信息？',pchar(Application.Title),MB_YESNOCANCEL+MB_DEFBUTTON1+MB_ICONINFORMATION);
      if i=6 then
         begin
            SaveOrder;
            OnSave(AObj);
...
begin
  ...
  try
    ...
    rs.SQL.Text :=
       ...
       'from ACC_RECVABLE_INFO A,VIW_CUSTOMER B,CA_SHOP_INFO C where A.TENANT_ID=C.TENANT_ID and A.SHOP_ID=C.SHOP_ID and A.TENANT_ID=B.TENANT_ID and A.CLIENT_ID=B.CLIENT_ID and A.TENANT_ID='+inttostr(Global.TENANT_ID)+' and A.CLIENT_ID='''+inttostr(Global.TENANT_ID)+''' and A.RECK_MNY<>0 and A.RECV_TYPE=''4'' order by ABLE_ID';
    Factor.Open(rs);
    cdsDetail.First;
    while not cdsDetail.Eof do cdsDetail.Delete;
    rs.first;
