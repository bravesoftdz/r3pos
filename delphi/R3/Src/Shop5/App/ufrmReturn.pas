...
implementation
uses uGlobal,uDsUtil,uFnUtil, uShopUtil, ufrmBasic, ufrmAccountInfo, ufrmCodeInfo,
  uShopGlobal;
{$R *.dfm}
...
begin
  ...
  begin
    if edtIC_AMONEY.CanFocus then edtIC_AMONEY.SetFocus;
    Raise Exception.Create('退款金额不能为0!');
  end;
  ...
  try
    ...
    rs1.FieldbyName('RECV_MNY').AsFloat := cdsTable1.FieldByName(Str_Pay).AsFloat;
    ...
    rs1.FieldbyName('REMARK').AsString := '储值卡退款';
    rs1.Post;
    rs2.Append;
    ...
    rs2.FieldbyName('RECV_TYPE').AsString := '6';
    ...
    Params.ParamByName('RECV_TYPE').AsString := '6';
    Params.ParamByName('ACCT_INFO').AsString := '储值卡退款';
    Factor.BeginBatch;
    try
...
procedure TfrmReturn.edtITEM_IDAddClick(Sender: TObject);
var
  r:TRecord_;
begin
  inherited;
...
procedure TfrmReturn.edtACCOUNT_IDAddClick(Sender: TObject);
var
  r:TRecord_;
begin
  inherited;
