...
begin
  ...
  result := 'select A.TENANT_ID,A.INDE_ID,A.GLIDE_NO,A.SALBILL_STATUS,A.INDE_DATE,A.PLAN_DATE,A.LINKMAN,A.SEND_ADDR,A.REMARK,A.INVOICE_FLAG,A.CLIENT_ID,A.CREA_USER,A.SHOP_ID,A.GUIDE_USER,'+
            ...
            ' from SAL_INVOICE_INFO H left join SAL_INVOICE_LIST I on H.TENANT_ID=I.TENANT_ID and H.INVD_ID=I.INVD_ID) B on A.TENANT_ID=B.TENANT_ID and A.INDE_ID=B.FROM_ID '+
...
begin
  ...
  result :=
   ...
   'select '+TopCnd+' A.TENANT_ID,A.SHOP_ID,A.DEPT_ID,A.INDE_ID,A.GLIDE_NO,A.INDE_DATE,A.PLAN_DATE,A.LINKMAN,A.TELEPHONE,A.SEND_ADDR,A.CLIENT_ID,A.CREA_USER,A.GUIDE_USER,'+
   'A.CHK_DATE,A.CHK_USER,A.FIG_ID,A.INDE_AMT,A.INDE_MNY,A.OTH1_MNY,A.OTH2_MNY,A.OTH3_MNY,A.OTH4_MNY,A.OTH5_MNY,A.PRINT_TIMES,'+
   'A.REMARK,A.INVOICE_FLAG,A.TAX_RATE,A.CREA_DATE,A.SALES_STYLE,A.BOND_MNY as ALL_BOND_MNY,A.BOND_RET as ALL_BOND_RET,'+
   'B.AMOUNT,B.APRICE,B.SEQNO,B.ORG_PRICE,B.PROPERTY_01,B.PROPERTY_02,B.UNIT_ID,B.BATCH_NO,B.LOCUS_NO,B.GODS_ID,B.CALC_MONEY,B.BARTER_INTEGRAL,B.AGIO_RATE,B.AGIO_MONEY,B.IS_PRESENT,B.REMARK as REMARK_DETAIL,'+
   ' B.BOND_MNY,B.BOND_RET from SAL_INDENTORDER A,SAL_INDENTDATA B '+
...
//  if not ShopGlobal.GetChkRight('100002314',2) then Raise Exception.Create('你没有开票的权限,请和管理员联系.');
  if CurOrder<>nil then
     begin
       ...
       if TfrmSalIndentOrder(CurOrder).cdsHeader.FieldByName('SEND_ADDR').AsString <> '' then
       ...
       if TfrmSalIndentOrder(CurOrder).cdsHeader.FieldByName('LINKMAN').AsString <> '' then
          LinkMan := TfrmSalIndentOrder(CurOrder).cdsHeader.FieldByName('LINKMAN').AsString
       else
       begin
          rs1 := ShopGlobal.GetZQueryFromName('PUB_CUSTOMER');
          if rs1.Locate('CLIENT_ID',Client_Id,[]) then
