...
type
  TfrmSalRetuOrder = class(TframeOrderForm)
...
procedure TfrmSalRetuOrder.CancelOrder;
begin
  inherited;
...
procedure TfrmSalRetuOrder.DeleteOrder;
begin
  inherited;
  Saved := false;
...
procedure TfrmSalRetuOrder.EditOrder;
begin
  inherited;
...
procedure TfrmSalRetuOrder.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmSalRetuOrder.NewOrder;
var
  rs:TZQuery;
begin
  inherited;
...
procedure TfrmSalRetuOrder.Open(id: string);
var
  Params:TftParamList;
begin
  inherited;
  ...
  try
    ...
    Factor.BeginBatch;
    try
      Factor.AddBatch(cdsHeader,'TSalRetuOrder',Params);
...
procedure TfrmSalRetuOrder.SaveOrder;
var
  Printed:boolean;
begin
  inherited;
  ...
  Saved := false;
  if edtSALES_DATE.EditValue = null then Raise Exception.Create('退货日期不能为空');
  ...
  edtLINKMAN.Text:=GetTrimStr(edtLINKMAN.Text,20); //2012.05.13截掉超长
...
begin
  ...
  begin
    Value := TColumnEh(Sender).Field.asFloat;
    Text := TColumnEh(Sender).Field.AsString;
    MessageBox(Handle,pchar('你没有修改退货单价格的权限,请和管理员联系...'),pchar(Application.Title),MB_OK+MB_ICONINFORMATION);
  end;
end;
...
procedure TfrmSalRetuOrder.edtCLIENT_IDSaveValue(Sender: TObject);
var
  rs,tmp:TZQuery;
begin
  inherited;
...
procedure TfrmSalRetuOrder.edtINVOICE_FLAGPropertiesChange(Sender: TObject);
begin
  inherited;
  if Locked then Exit;
  if dbState=dsBrowse then Exit;
...
procedure TfrmSalRetuOrder.AuditOrder;
var
  Msg :string;
  Params:TftParamList;
begin
...
procedure TfrmSalRetuOrder.fndGODS_IDSaveValue(Sender: TObject);
begin
...
procedure TfrmSalRetuOrder.edtCLIENT_IDAddClick(Sender: TObject);
var r:TRecord_;
begin
  inherited;
  r := TRecord_.Create;
...
procedure TfrmSalRetuOrder.AgioInfo(id: string);
begin
  inherited;

end;

procedure TfrmSalRetuOrder.WriteInfo(id: string);
var
  rs:TZQuery;
  SObj:TRecord_;
begin
...
function TfrmSalRetuOrder.OpenDialogCustomer(KeyString: string): boolean;
begin
  result := false;
  if dbState = dsBrowse then Exit;
...
procedure TfrmSalRetuOrder.ReadFrom(DataSet: TDataSet);
begin
  inherited
end;

procedure TfrmSalRetuOrder.ShowOweInfo;
var
  rs:TZQuery;
begin
  rs := TZQuery.Create(nil);
...
procedure TfrmSalRetuOrder.ShowInfo;
var
  rs,bs:TZQuery;
begin
...
procedure TfrmSalRetuOrder.UnitToCalc(UNIT_ID: string);
begin
  inherited;
  ShowInfo;
end;
...
procedure TfrmSalRetuOrder.edtTableAfterScroll(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmSalRetuOrder.edtSALES_STYLEAddClick(Sender: TObject);
var
  r:TRecord_;
begin
  inherited;
...
procedure TfrmSalRetuOrder.edtCLIENT_IDFindClick(Sender: TObject);
begin
  inherited;
  if dbState = dsBrowse then Exit;
  OpenDialogCustomer('');
...
procedure TfrmSalRetuOrder.edtCLIENT_IDPropertiesChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmSalRetuOrder.WMFillData(var Message: TMessage);
var
  frmSalesOrder:TfrmSalesOrder;
  i:integer;
begin
  if dbState <> dsInsert then Raise Exception.Create('不是在新增状态不能完成操作');
  frmSalesOrder := TfrmSalesOrder(Message.WParam);
  with TfrmSalesOrder(frmSalesOrder) do
    begin
...
procedure TfrmSalRetuOrder.edtGUIDE_USERAddClick(Sender: TObject);
var
  r:TRecord_;
begin
  inherited;
...
function TfrmSalRetuOrder.getLvlPrice(priceDataSet : TZQuery; number : currency) : currency;
...
procedure TfrmSalRetuOrder.useLvlPriceClick(Sender: TObject);
var SQL : string;
...
procedure TfrmSalRetuOrder.FilterUserClick(Sender: TObject);
begin
...
procedure TfrmSalRetuOrder.GetUserInfo(Aobj_: TRecord_);
begin
...
procedure TfrmSalRetuOrder.edtLINKMANKeyDown(Sender: TObject;
...
procedure TfrmSalRetuOrder.edtTELEPHONEKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  if (ssCtrl in Shift) and (Key = VK_RETURN) then
...
procedure TfrmSalRetuOrder.edtSEND_ADDRKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  if (ssCtrl in Shift) and (Key = VK_RETURN) then
