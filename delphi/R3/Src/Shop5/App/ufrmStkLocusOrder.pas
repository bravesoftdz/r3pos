...
begin
  inherited;
  if cdsHeader.IsEmpty then Raise Exception.Create('不能修改空单据');
  if IsAudit then Raise Exception.Create('已经审核的单据不能再收货.');
...
begin
  ...
  if IsAudit then
     begin
       ...
       if cdsHeader.FieldByName('LOCUS_CHK_USER').AsString<>Global.UserID then Raise Exception.Create('只有审核人才能对当前进货单执行弃审');
  ...
  try
    ...
    try
      ...
      if not IsAudit then
         Msg := Factor.ExecProc('TStockLocusNoAudit',Params)
      else
         Msg := Factor.ExecProc('TStockLocusNoUnAudit',Params) ;
    finally
       Params.free;
    end;
...
function TfrmStkLocusOrder.CalcCurr: real;
var
  amt:integer;
begin
