...
type
  TfrmSupPayDayReport = class(TframeBaseReport)
...
procedure TfrmSupPayDayReport.FormCreate(Sender: TObject);
begin
  inherited;
  IsOnDblClick:=False;
...
function TfrmSupPayDayReport.GetGroupSQL(chk:boolean=true): string;
var
...
function TfrmSupPayDayReport.GetRowType: integer;
begin
  result :=0;  // DBGridEh.DataSource.DataSet.FieldbyName('grp0').AsInteger;
end;

procedure TfrmSupPayDayReport.actFindExecute(Sender: TObject);
var strSql: string;
begin
...
function TfrmSupPayDayReport.GetShopSQL(chk:boolean=true): string;
var
  strSql,RecvData,strWhere: string;
begin
...
function TfrmSupPayDayReport.GetSortSQL(chk:boolean=true): string;
var
  strSql,RecvData,strWhere: string;
begin
...
function TfrmSupPayDayReport.GetRecvSupplieSQL(chk:boolean=true): string;
var
  strSql,RecvData,strWhere: string;
begin
...
procedure TfrmSupPayDayReport.DBGridEh1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupPayDayReport.DBGridEh2DblClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupPayDayReport.DBGridEh3DblClick(Sender: TObject);
var
  CurDate: string;
begin
  inherited;
  ...
  IsOnDblClick:=true; //设置标记位
  CurDate:=trim(adoReport3.fieldbyName('ABLE_DATE').AsString);
...
procedure TfrmSupPayDayReport.FormDestroy(Sender: TObject);
begin
  TDbGridEhSort.FreeForm(self);
  inherited;
end;

procedure TfrmSupPayDayReport.InitGrid;
...
procedure TfrmSupPayDayReport.PrintBefore;
var
  ReStr: string;
  Title: TStringList;
begin
...
function TfrmSupPayDayReport.AddReportReport(TitleList: TStringList; PageNo: string): string;
var
...
function TfrmSupPayDayReport.GetShopIDCnd(ShopID: TzrComboBoxList; FieldName: string): string;
begin
  result:='';
...
function TfrmSupPayDayReport.GetDateCnd(BegDate, EndDate: TcxDateEdit; FieldName: string): string;
begin
  result:='';
...
function TfrmSupPayDayReport.GetShopGroupCnd(SHOP_TYPE: TcxComboBox; TYPE_VALUE: TzrComboBoxList; AliasName: string): string;
var
  AName: string;
begin
  result:='';
...
function TfrmSupPayDayReport.GetRckMaxDate(EndDate: integer): integer;
var
  rs:TZQuery;
begin
...
function TfrmSupPayDayReport.GetRcevBaseData(fndBegDate, fndEndDate: TcxDateEdit): string;
var
...
function TfrmSupPayDayReport.GetGuideSQl(chk: boolean): string;
var
...
begin
  ...
  strSql:=
    ...
    '(select A.* from VIW_SUPPAYDATA A,CA_SHOP_INFO B '+
    ' where A.TENANT_ID=B.TENANT_ID and A.SHOP_ID=B.SHOP_ID and A.TENANT_ID='+InttoStr(Global.TENANT_ID)+
    ' '+strWhere+') jb '+
...
procedure TfrmSupPayDayReport.DBGridEh4DblClick(Sender: TObject);
begin
...
procedure TfrmSupPayDayReport.DBGridEh1GetFooterParams(Sender: TObject;
...
procedure TfrmSupPayDayReport.DBGridEh2GetFooterParams(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; AFont: TFont;
  var Background: TColor; var Alignment: TAlignment; State: TGridDrawState;
  var Text: String);
begin
...
procedure TfrmSupPayDayReport.DBGridEh3GetFooterParams(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; AFont: TFont;
  var Background: TColor; var Alignment: TAlignment; State: TGridDrawState;
  var Text: String);
begin
...
procedure TfrmSupPayDayReport.DBGridEh4GetFooterParams(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; AFont: TFont;
  var Background: TColor; var Alignment: TAlignment; State: TGridDrawState;
  var Text: String);
begin
...
procedure TfrmSupPayDayReport.DBGridEh5GetFooterParams(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; AFont: TFont;
  var Background: TColor; var Alignment: TAlignment; State: TGridDrawState;
  var Text: String);
begin
