...
begin
  ...
  SelectSQL.Text := 'select g.*,h.CHANGE_TYPE as CHANGE_TYPE from ('+
               ...
               'j.REMARK from STO_CHANGEDATA j left outer join VIW_GOODSINFO b on j.TENANT_ID=b.TENANT_ID and j.GODS_ID=b.GODS_ID where j.TENANT_ID=:TENANT_ID and j.CHANGE_ID=:CHANGE_ID ) g '+
...
begin
  ...
  SelectSQL.Text :=
               ...
               ' select CHANGE_ID,TENANT_ID,SHOP_ID,GLIDE_NO,CHANGE_DATE,CHANGE_TYPE,CHANGE_CODE,DUTY_USER,REMARK,CHK_USER,CHK_DATE,FROM_ID,CREA_DATE,CHANGE_AMT,CHANGE_MNY,CREA_USER,COMM,'+
  ...
  IsSQLUpdate := True;
  Str := 'insert into STO_CHANGEORDER(CHANGE_ID,TENANT_ID,SHOP_ID,GLIDE_NO,CHANGE_DATE,CHANGE_TYPE,CHANGE_CODE,DUTY_USER,REMARK,CHK_USER,CHK_DATE,FROM_ID,CHANGE_AMT,CHANGE_MNY,CREA_DATE,CREA_USER,COMM,TIME_STAMP) '
    + 'VALUES(:CHANGE_ID,:TENANT_ID,:SHOP_ID,:GLIDE_NO,:CHANGE_DATE,:CHANGE_TYPE,:CHANGE_CODE,:DUTY_USER,:REMARK,:CHK_USER,:CHK_DATE,:FROM_ID,:CHANGE_AMT,:CHANGE_MNY,:CREA_DATE,:CREA_USER,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  Str := 'update STO_CHANGEORDER set CHANGE_ID=:CHANGE_ID,TENANT_ID=:TENANT_ID,SHOP_ID=:SHOP_ID,GLIDE_NO=:GLIDE_NO,CHANGE_DATE=:CHANGE_DATE,CHANGE_TYPE=:CHANGE_TYPE,CHANGE_CODE=:CHANGE_CODE,DUTY_USER=:DUTY_USER,REMARK=:REMARK,'+
      'CHK_USER=:CHK_USER,CHK_DATE=:CHK_DATE,FROM_ID=:FROM_ID,CHANGE_AMT=:CHANGE_AMT,CHANGE_MNY=:CHANGE_MNY,CREA_DATE=:CREA_DATE,CREA_USER=:CREA_USER,'
...
begin
  try
    Str := 'update STO_CHANGEORDER set CHK_DATE='''+Params.FindParam('CHK_DATE').asString+''',CHK_USER='''+Params.FindParam('CHK_USER').asString+''' where TENANT_ID='+Params.FindParam('TENANT_ID').asString +' and CHANGE_ID='''+Params.FindParam('CHANGE_ID').asString+''' and CHK_DATE IS NULL';
...
begin
   try
    Str := 'update STO_CHANGEORDER set CHK_DATE=null,CHK_USER=null where TENANT_ID='+Params.FindParam('TENANT_ID').asString +' and CHANGE_ID='''+Params.FindParam('CHANGE_ID').asString+''' and CHK_DATE IS NOT NULL';
    n := AGlobal.ExecSQL(Str);
    if n=0 then
