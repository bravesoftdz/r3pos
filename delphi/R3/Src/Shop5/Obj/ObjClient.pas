...
begin
  try
    ...
    try
      Temp.Close;
      Temp.SQL.Text := 'select * from PUB_IC_INFO where COMM not in (''02'',''12'') and IC_CARDNO=:OLD_IC_CARDNO and TENANT_ID=:OLD_TENANT_ID';
      AGlobal.Open(Temp);
      if Temp.RecordCount > 0 then
        Raise Exception.Create('此卡号已经存在，不能重复！');
      ...
      'IC_TYPE=:IC_TYPE,ACCU_INTEGRAL=:ACCU_INTEGRAL,RULE_INTEGRAL=:RULE_INTEGRAL,INTEGRAL=:INTEGRAL,BALANCE=:BALANCE,PASSWRD=:PASSWRD,'+
      'USING_DATE=:USING_DATE,TIME_STAMP='+GetTimeStamp(AGlobal.iDbType)+' where CLIENT_ID=:OLD_CLIENT_ID and TENANT_ID=:OLD_TENANT_ID and UNION_ID=:OLD_UNION_ID';
      AGlobal.ExecSQL(Str,Self);
      Result := True;
    finally
      Temp.Free;
