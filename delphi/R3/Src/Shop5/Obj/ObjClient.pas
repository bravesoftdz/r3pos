...
begin
  ...
  +'INTEGRAL,BALANCE,SHOP_ID from PUB_CLIENTINFO where CLIENT_ID=:CLIENT_ID and TENANT_ID=:TENANT_ID order by CLIENT_ID';
  IsSQLUpdate := True;
...
begin
  ...
  AGlobal.BeginTrans;
  try
    AGlobal.ExecSQL('update PUB_CLIENTINFO set  COMM=''02'',TIME_STAMP='+GetTimeStamp(AGlobal.iDbType)+' where COMP_ID='
    +QuotedStr(Params.ParamByName('COMP_ID').asString)+' and CLient_ID='+QuotedStr(Params.ParamByName('CLIENT_ID').asString));
    AGlobal.CommitTrans;
    Result:=True;
  except
    on E:Exception do
