  ...
  if FieldbyName('PLAN_DATE').AsString <> '' then
     begin
       Str := 'insert into STK_STOCKDATA(TENANT_ID,SHOP_ID,SEQNO,STOCK_ID,GODS_ID,BATCH_NO,LOCUS_NO,BOM_ID,PROPERTY_01,PROPERTY_02,UNIT_ID,AMOUNT,ORG_PRICE,IS_PRESENT,APRICE,AMONEY,AGIO_RATE,AGIO_MONEY,CALC_AMOUNT,CALC_MONEY,REMARK) '
            + 'VALUES(:TENANT_ID,:CLIENT_ID,:SEQNO,:SALES_ID,:GODS_ID,:BATCH_NO,:LOCUS_NO,:BOM_ID,:PROPERTY_01,:PROPERTY_02,:UNIT_ID,:AMOUNT,:APRICE,:IS_PRESENT,:COST_PRICE,round(:CALC_AMOUNT*:COST_PRICE,2),0,0,:CALC_AMOUNT,round(:CALC_AMOUNT*:COST_PRICE,2),:REMARK)';
...
begin
  ...
  begin
     if (FieldbyName('GLIDE_NO').AsString='') or (Pos('新增',FieldbyName('GLIDE_NO').AsString)>0) then
        FieldbyName('GLIDE_NO').AsString := GetSequence(AGlobal,'GNO_B_'+FieldbyName('SHOP_ID').AsString,FieldbyName('TENANT_ID').AsString,formatDatetime('YYMMDD',now()),5);
  end;
  if FieldbyName('PLAN_DATE').AsString <> '' then
  begin
...
function TDbOrder.CheckTimeStamp(aGlobal: IdbHelp; s: string): boolean;
var
  rs:TZQuery;
begin
  rs := TZQuery.Create(nil);
