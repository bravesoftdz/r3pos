...
begin
  inherited;
  SelectSQL.Text :=
  ' select ROWS_ID,TENANT_ID,REPORT_ID,CELL_TYPE,COL,ROW,DISPLAY_NAME,FIELD_NAME,'' as FIELD_NAME_TEXT,INDEX_ID,SUM_TYPE,INDEX_FLAG,SUB_FLAG '+
  ' from SYS_REPORT_TEMPLATE where TENANT_ID=:TENANT_ID and REPORT_ID=:REPORT_ID and CELL_TYPE=:CELL_TYPE ';
  IsSQLUpdate := True;

  Str := 'insert into SYS_REPORT_TEMPLATE(ROWS_ID,TENANT_ID,REPORT_ID,CELL_TYPE,COL,ROW,DISPLAY_NAME,FIELD_NAME,INDEX_ID,SUM_TYPE,INDEX_FLAG,SUB_FLAG) '+
  ' VALUES(:ROWS_ID,:TENANT_ID,:REPORT_ID,:CELL_TYPE,:COL,:ROW,:DISPLAY_NAME,:FIELD_NAME,:INDEX_ID,:SUM_TYPE,:INDEX_FLAG,:SUB_FLAG)';
  InsertSQL.Text := Str;
