...
begin
  ...
  KeyFields:='DEPT_ID';
  //初始化查询
  Str:='select DEPT_ID,DEPT_NAME,LEVEL_ID,DEPT_SPELL,TENANT_ID,TELEPHONE,LINKMAN,DEPT_TYPE,FAXES,REMARK,SEQ_NO,SubStr(LEVEL_ID,1,Length(LEVEL_ID)-3) as UPDEPT_ID '+
       ' From CA_DEPT_INFO Where TENANT_ID=:TENANT_ID and DEPT_ID=:DEPT_ID ';
  SelectSQL.Text:=ParseSQL(iDbType,Str);

  Str :='insert into CA_DEPT_INFO (DEPT_ID,DEPT_NAME,LEVEL_ID,DEPT_SPELL,TENANT_ID,TELEPHONE,LINKMAN,DEPT_TYPE,FAXES,REMARK,SEQ_NO,COMM,TIME_STAMP) '+
        ' values (:DEPT_ID,:DEPT_NAME,:LEVEL_ID,:DEPT_SPELL,:TENANT_ID,:TELEPHONE,:LINKMAN,:DEPT_TYPE,:FAXES,:REMARK,:SEQ_NO,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Add(Str);
  ...
  Str :='update CA_DEPT_INFO set DEPT_ID=:DEPT_ID,DEPT_NAME=:DEPT_NAME,DEPT_SPELL=:DEPT_SPELL,TELEPHONE=:TELEPHONE,LINKMAN=:LINKMAN,FAXES=:FAXES,REMARK=:REMARK,SEQ_NO=:SEQ_NO,'+
        'DEPT_TYPE=:DEPT_TYPE,COMM='+ GetCommStr(iDbType)+','+ 'TIME_STAMP='+GetTimeStamp(iDbType)+' where TENANT_ID=:OLD_TENANT_ID and DEPT_ID=:OLD_DEPT_ID';
  UpdateSQL.Add(Str);
