...
begin
  ...
  Str := 'insert into MKT_KPI_OPTION(TENANT_ID,KPI_ID,SEQNO,KPI_RATE,KPI_AMT,KPI_DATE1,KPI_DATE2,KPI_AGIO,USING_BRRW,COMM,TIME_STAMP) '+
         ' VALUES(:TENANT_ID,:KPI_ID,:SEQNO,:KPI_RATE,:KPI_AMT,:KPI_DATE1,:KPI_DATE2,:KPI_AGIO,:USING_BRRW,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Add( Str);
  Str := 'update MKT_KPI_OPTION set TENANT_ID=:TENANT_ID,KPI_ID=:KPI_ID,SEQNO=:SEQNO,KPI_RATE=:KPI_RATE,KPI_AMT=:KPI_AMT,'+
         'KPI_DATE1=:KPI_DATE1,KPI_DATE2=:KPI_DATE2,KPI_AGIO=:KPI_AGIO,USING_BRRW=:USING_BRRW,COMM='+GetCommStr(iDbType)+
         ',TIME_STAMP='+GetTimeStamp(iDbType)+' where KPI_ID=:OLD_KPI_ID and TENANT_ID=:OLD_TENANT_ID and SEQNO=:OLD_SEQNO ';
  UpdateSQL.Add( Str);
  Str := 'delete from MKT_KPI_OPTION where KPI_ID=:OLD_KPI_ID and TENANT_ID=:OLD_TENANT_ID and SEQNO=:OLD_SEQNO ';
  DeleteSQL.Add( Str);
end;
