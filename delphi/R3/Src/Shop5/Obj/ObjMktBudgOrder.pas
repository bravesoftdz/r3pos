...
begin
  inherited;
  Str :=
  'select C.TENANT_ID,C.BUDG_ID,C.SHOP_ID,C.SEQNO,C.ACTIVE_ID,C.KPI_ID,D.BUDG_MNY,E.ACTIVE_NAME as ACTIVE_ID_TEXT,F.KPI_NAME as KPI_ID_TEXT,'+
  'D.BUDG_MNY-ifnull(D.BUDG_VRF,0)+C.BUDG_VRF as LESS_MNY,C.BUDG_VRF,D.BUDG_MNY-ifnull(D.BUDG_VRF,0) as BLAN_MNY,C.REMARK from ('+
  'select A.TENANT_ID,A.BUDG_ID,B.REQU_ID,A.SHOP_ID,A.SEQNO,A.ACTIVE_ID,A.KPI_ID,A.BUDG_VRF,A.REMARK from MKT_BUDGDATA A,MKT_BUDGORDER B '+
  ' where A.TENANT_ID=B.TENANT_ID and A.BUDG_ID=B.BUDG_ID and A.TENANT_ID=:TENANT_ID and A.BUDG_ID=:BUDG_ID '+
  ' ) C left join (select TENANT_ID,REQU_ID,KPI_ID,sum(isnull(BUDG_MNY,0)) as BUDG_MNY,sum(isnull(BUDG_VRF,0)) as BUDG_VRF '+
  ' from MKT_REQUDATA group by TENANT_ID,REQU_ID,KPI_ID) D on C.TENANT_ID=D.TENANT_ID and C.REQU_ID=D.REQU_ID and C.KPI_ID=D.KPI_ID '+
  ' left join MKT_ACTIVE_INFO E on C.TENANT_ID=E.TENANT_ID and C.ACTIVE_ID=E.ACTIVE_ID '+
  ' left join MKT_KPI_INDEX F on C.TENANT_ID=F.TENANT_ID and C.KPI_ID=F.KPI_ID ';
