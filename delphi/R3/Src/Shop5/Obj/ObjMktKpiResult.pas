...
begin
  inherited;
  SelectSQL.Text := 'select A.ROWS_ID,A.TENANT_ID,A.CLIENT_ID,A.KPI_ID,A.KPI_YEAR,(A.KPI_YEAR*10000+A.KPI_DATE1) as KPI_DATE1,'+
                    'case when A.KPI_DATE1>A.KPI_DATE2 then ((A.KPI_YEAR+1)*10000+A.KPI_DATE2) else (A.KPI_YEAR*10000+A.KPI_DATE2) end as KPI_DATE2,'+
  ...
  Str := 'update MKT_KPI_RESULT_LIST set TENANT_ID=:TENANT_ID,CLIENT_ID=:CLIENT_ID,KPI_ID=:KPI_ID,KPI_YEAR=:KPI_YEAR,KPI_DATE1=:KPI_DATE1,'+
         ...
         'ACTR_RATIO=:ACTR_RATIO,KPI_MNY=:KPI_MNY,BUDG_KPI=:BUDG_KPI where ROWS_ID=:OLD_ROWS_ID ';
  UpdateSQL.Text := Str;
