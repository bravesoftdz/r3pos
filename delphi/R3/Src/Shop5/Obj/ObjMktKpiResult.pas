...
begin
  ...
  Str := ' update MKT_KPI_RESULT set FISH_AMT=:FISH_AMT,ADJS_AMT=:ADJS_AMT,FISH_MNY=:FISH_MNY,ADJS_MNY=:ADJS_MNY,KPI_MNY=:KPI_MNY,'+
         ' BUDG_KPI=:BUDG_KPI,CREA_DATE=:CREA_DATE,CREA_USER=:CREA_USER '+
         ' where TENANT_ID=:OLD_TENANT_ID and KPI_YEAR=:OLD_KPI_YEAR and KPI_ID=:OLD_KPI_ID and CLIENT_ID=:OLD_CLIENT_ID ';
  UpdateSQL.Text := Str;
...
REMARK,CREA_DATE,CREA_USER,COMM,TIME_STAMP}
    ...
    ExceSql := 'insert into MKT_KPI_RESULT (TENANT_ID,CLIENT_ID,KPI_ID,KPI_YEAR,SHOP_ID,DEPT_ID,PLAN_ID,IDX_TYPE,KPI_TYPE,'+
               ...
               'select B.TENANT_ID,B.CLIENT_ID,A.KPI_ID,B.KPI_YEAR,B.SHOP_ID,B.DEPT_ID,B.PLAN_ID,A.IDX_TYPE,A.KPI_TYPE,:CREA_DATE,'+
               ':CREA_USER, C.AMOUNT,C.AMONEY,0 as FISH_AMT,0 as ADJS_AMT,0 as FISH_MNY,0 as ADJS_MNY,0 as KPI_MNY,0 as WDW_MNY,C.REMARK,''00'','+GetTimeStamp(AGlobal.iDbType)+
...
begin
  ...
  SelectSQL.Text :=
             ParseSQL(iDbType,
                    ...
                    'case when A.KPI_DATA in (''1'',''4'') then C.UNIT_NAME else ''元'' end as UNIT_NAME,'+
