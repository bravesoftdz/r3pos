...
begin
  ...
  try
    ...
    if rs.FieldByName('Record_Sum').AsInteger > 0 then
       Raise Exception.Create('该经销商在"'+FieldbyName('KPI_YEAR').AsString+'"年度已经有销售计划..');
  finally
    rs.Free;
  end;
  ...
  begin
     AGlobal.ExecSQL(
       ...
       + 'VALUES('''+newid(Params.ParambyName('SHOP_ID').AsString)+''',:TENANT_ID,'''+FieldbyName('TENANT_ID').AsString+'0001'',:CLIENT_ID,'''+'固定保证金【合同号'+FieldbyName('GLIDE_NO').AsString+'】'+''',''5'',:BOND_MNY,0,0,:BOND_MNY,:PLAN_DATE,:PLAN_ID,:CREA_DATE,:CREA_USER,''00'','+GetTimeStamp(iDbType)+')'
    ,self);
  end;
...
begin
  ...
  //修改时处理应收账款
  AGlobal.ExecSQL('update ACC_RECVABLE_INFO set ACCT_MNY=:BOND_MNY,RECK_MNY=:BOND_MNY - RECV_MNY - REVE_MNY,CLIENT_ID=:CLIENT_ID,ABLE_DATE=:PLAN_DATE,SALES_ID=:PLAN_ID  where TENANT_ID=:OLD_TENANT_ID and SALES_ID=:OLD_PLAN_ID and RECV_TYPE=''5''',self);
end;
...
begin
  ...
  SelectSQL.Text :=
     ...
     'B.CLIENT_NAME,A.DEPT_ID,F.DEPT_NAME as DEPT_ID_TEXT,A.PLAN_USER,D.USER_NAME as PLAN_USER_TEXT,A.SHOP_ID,G.SHOP_NAME as SHOP_ID_TEXT,'+
     ...
     ' left join CA_SHOP_INFO G on A.TENANT_ID=G.TENANT_ID and A.SHOP_ID=G.SHOP_ID where A.TENANT_ID=:TENANT_ID and A.PLAN_ID=:PLAN_ID';
  IsSQLUpdate := True;
  Str := 'insert into MKT_PLANORDER(TENANT_ID,PLAN_ID,SHOP_ID,PLAN_TYPE,GLIDE_NO,KPI_YEAR,PLAN_DATE,BEGIN_DATE,END_DATE,CLIENT_ID,DEPT_ID,PLAN_USER,CHK_DATE,CHK_USER,PLAN_AMT,PLAN_MNY,BOND_MNY,BUDG_MNY,REMARK,CREA_DATE,CREA_USER,COMM,TIME_STAMP) '
    + 'VALUES(:TENANT_ID,:PLAN_ID,:SHOP_ID,:PLAN_TYPE,:GLIDE_NO,:KPI_YEAR,:PLAN_DATE,:BEGIN_DATE,:END_DATE,:CLIENT_ID,:DEPT_ID,:PLAN_USER,:CHK_DATE,:CHK_USER,:PLAN_AMT,:PLAN_MNY,:BOND_MNY,:BUDG_MNY,:REMARK,:CREA_DATE,:CREA_USER,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  Str := 'update MKT_PLANORDER set TENANT_ID=:TENANT_ID,PLAN_ID=:PLAN_ID,SHOP_ID=:SHOP_ID,PLAN_TYPE=:PLAN_TYPE,GLIDE_NO=:GLIDE_NO,KPI_YEAR=:KPI_YEAR,PLAN_DATE=:PLAN_DATE,BEGIN_DATE=:BEGIN_DATE,END_DATE=:END_DATE,'+
...
begin
  inherited;
  SelectSQL.Text :=
     'select A.TENANT_ID,A.SEQNO,A.PLAN_ID,A.SHOP_ID,A.KPI_ID,B.KPI_NAME as KPI_ID_TEXT,A.AMOUNT,A.AMONEY,A.BOND_MNY,A.REMARK '+
  ...
  IsSQLUpdate := True;
  Str := 'insert into MKT_PLANDATA(TENANT_ID,SEQNO,PLAN_ID,SHOP_ID,KPI_ID,AMOUNT,AMONEY,BOND_MNY,REMARK) '
    + 'VALUES(:TENANT_ID,:SEQNO,:PLAN_ID,:SHOP_ID,:KPI_ID,:AMOUNT,:AMONEY,:BOND_MNY,:REMARK)';
  InsertSQL.Text := Str;                                                          
  Str := 'update MKT_PLANDATA set TENANT_ID=:TENANT_ID,SEQNO=:SEQNO,PLAN_ID=:PLAN_ID,SHOP_ID=:SHOP_ID,KPI_ID=:KPI_ID,AMOUNT=:AMOUNT,AMONEY=:AMONEY,BOND_MNY=:BOND_MNY,REMARK=:REMARK '
