...
begin
  ...
  'select A.PAY_ID,A.SHOP_ID,A.DEPT_ID,A.TENANT_ID,A.GLIDE_NO,A.CLIENT_ID,A.ACCOUNT_ID,B.ACCT_NAME as ACCOUNT_ID_TEXT,A.PAYM_ID,A.PAY_MNY,A.ITEM_ID,A.PAY_DATE,A.PAY_USER,A.CHK_DATE,A.CHK_USER,A.REMARK,A.COMM,A.CREA_DATE,A.BILL_NO,A.CREA_USER,A.TIME_STAMP '+
  ...
  'left outer join VIW_CLIENTINFO e on je.TENANT_ID=e.TENANT_ID and je.CLIENT_ID=e.CLIENT_ID ) jf '+
  'left outer join CA_DEPT_INFO f on jf.TENANT_ID=f.TENANT_ID and jf.DEPT_ID=f.DEPT_ID ';
  IsSQLUpdate := True;
  Str := 'insert into ACC_PAYORDER(PAY_ID,TENANT_ID,SHOP_ID,DEPT_ID,GLIDE_NO,ACCOUNT_ID,PAYM_ID,PAY_MNY,CLIENT_ID,ITEM_ID,PAY_DATE,PAY_USER,CHK_DATE,CHK_USER,COMM,CREA_DATE,CREA_USER,BILL_NO,REMARK,TIME_STAMP) '
    + 'VALUES(:PAY_ID,:TENANT_ID,:SHOP_ID,:DEPT_ID,:GLIDE_NO,:ACCOUNT_ID,:PAYM_ID,:PAY_MNY,:CLIENT_ID,:ITEM_ID,:PAY_DATE,:PAY_USER,:CHK_DATE,:CHK_USER,''00'',:CREA_DATE,:CREA_USER,:BILL_NO,:REMARK,'+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  Str := 'update ACC_PAYORDER set PAY_ID=:PAY_ID,TENANT_ID=:TENANT_ID,SHOP_ID=:SHOP_ID,DEPT_ID=:DEPT_ID,GLIDE_NO=:GLIDE_NO,ACCOUNT_ID=:ACCOUNT_ID,PAYM_ID=:PAYM_ID,CLIENT_ID=:CLIENT_ID,ITEM_ID=:ITEM_ID,PAY_DATE=:PAY_DATE,'+
...
begin
  ...
  'select case when A.PAY_MNY=0 then 0 else 1 end as A,A.TENANT_ID,A.SHOP_ID,B.SHOP_ID as ACC_SHOP_ID,A.PAY_ID,A.SEQNO,A.ABLE_ID,A.ABLE_TYPE,'+
