      ...
      begin
        if copy(rs.FieldbyName('COMM').AsString,2,1)='2' then //如果原来删除的等级，重新启动原有编码
           begin
             ...
             case iDbType of
             0: AGlobal.ExecSQL('delete PUB_PRICEGRADE where PRICE_ID=:PRICE_ID ',self);
             3: AGlobal.ExecSQL('delete from PUB_PRICEGRADE where PRICE_ID=:PRICE_ID ',self);
             end;
           end
        else
...
begin
  ...
  'AGIO_SORTS,SEQ_NO,COMM,TIME_STAMP from PUB_PRICEGRADE where COMM not in (''02'',''12'') and TENANT_ID=:TENANT_ID order by PRICE_ID';
  IsSQLUpdate := True;
