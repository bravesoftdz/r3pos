...
begin
  inherited;
   ...
   'select a.TENANT_ID,a.QUESTION_ID,a.QUESTION_ITEM_ID,a.SEQ_NO,a.QUESTION_ITEM_TYPE,a.QUESTION_INFO,a.QUESTION_OPTIONS '+
  ...
  IsSQLUpdate := True;
  Str :=
  'insert into MSC_QUESTION_ITEM(TENANT_ID,QUESTION_ID,QUESTION_ITEM_ID,SEQ_NO,QUESTION_ITEM_TYPE,QUESTION_INFO,QUESTION_OPTIONS) '+
  'VALUES(:TENANT_ID,:QUESTION_ID,:QUESTION_ITEM_ID,:SEQ_NO,:QUESTION_ITEM_TYPE,:QUESTION_INFO,:QUESTION_OPTIONS)';
  InsertSQL.Text := Str;
  Str :=
  'update MSC_QUESTION_ITEM set SEQ_NO=:SEQ_NO,QUESTION_ITEM_TYPE=:QUESTION_ITEM_TYPE,QUESTION_INFO=:QUESTION_INFO,QUESTION_OPTIONS=:QUESTION_OPTIONS '+
...
begin
  inherited;
   ...
   'QUESTION_ITEM_AMT,REMARK,END_DATE from MSC_QUESTION where TENANT_ID=:TENANT_ID and QUESTION_ID=:QUESTION_ID';
  ...
  'ANSWER_FLAG,QUESTION_ITEM_AMT,REMARK,END_DATE,COMM,TIME_STAMP) '+
  ...
  ':QUESTION_ITEM_AMT,:REMARK,:END_DATE,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  ...
  ' where  COMM not in (''02'',''12'') TENANT_ID=:OLD_TENANT_ID and QUESTION_ID=:OLD_QUESTION_ID ';
  UpdateSQL.Text := Str;
  
  Str := 'update MSC_QUESTION set COMM=''02'',TIME_STAMP='+GetTimeStamp(iDbType)+
  ' where COMM not in (''02'',''12'') and TENANT_ID=:OLD_TENANT_ID and QUESTION_ID=:OLD_QUESTION_ID ';
  DeleteSQL.Text := Str;
...
begin
  inherited;
   SelectSQL.Text :=  
   'select TENANT_ID,QUESTION_ID,SHOP_ID,ANSWER_DATE,ANSWER_USER,ANSWER_USE_TIME,QUESTION_FEEDBACK_STATUS,QUESTION_ANSWER_STATUS '+
   'from MSC_INVEST_LIST where TENANT_ID=:TENANT_ID and QUESTION_ID=:QUESTION_ID and SHOP_ID=:SHOP_ID';

  IsSQLUpdate := True;
  Str :=
  'insert into MSC_INVEST_LIST(TENANT_ID,QUESTION_ID,SHOP_ID,ANSWER_DATE,ANSWER_USER,ANSWER_USE_TIME,QUESTION_FEEDBACK_STATUS,QUESTION_ANSWER_STATUS,COMM,TIME_STAMP) '+
  'VALUES(:TENANT_ID,:QUESTION_ID,:SHOP_ID,:ANSWER_DATE,:ANSWER_USER,:ANSWER_USE_TIME,:QUESTION_FEEDBACK_STATUS,:QUESTION_ANSWER_STATUS,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;

  Str :=
  'update MSC_INVEST_LIST set ANSWER_DATE=:ANSWER_DATE,ANSWER_USER=:ANSWER_USER,ANSWER_USE_TIME=:ANSWER_USE_TIME,QUESTION_FEEDBACK_STATUS=:QUESTION_FEEDBACK_STATUS,QUESTION_ANSWER_STATUS=:QUESTION_ANSWER_STATUS,'+
  'COMM='+GetCommStr(iDbType)+',TIME_STAMP='+GetTimeStamp(iDbType)+
...
begin
  inherited;
   ...
   'on a.TENANT_ID=b.TENANT_ID and a.QUESTION_ID=b.QUESTION_ID where a.TENANT_ID=:TENANT_ID and a.QUESTION_ID=:QUESTION_ID ';
  ...
  ' where TENANT_ID=:OLD_TENANT_ID and ROWS_ID=:OLD_ROWS_ID and SHOP_ID=:OLD_SHOP_ID ';
  UpdateSQL.Text := Str;
