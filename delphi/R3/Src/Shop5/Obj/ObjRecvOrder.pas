...
begin
  ...
  'select A.RECV_ID,A.TENANT_ID,A.SHOP_ID,A.GLIDE_NO,A.ACCOUNT_ID,A.PAYM_ID,A.CLIENT_ID,B.ACCT_NAME as ACCOUNT_ID_TEXT,A.ITEM_ID,'+
  ...
  IsSQLUpdate := True;
  Str := 'insert into ACC_RECVORDER(TENANT_ID,RECV_ID,SHOP_ID,GLIDE_NO,ACCOUNT_ID,PAYM_ID,RECV_MNY,CLIENT_ID,ITEM_ID,RECV_DATE,RECV_USER,CHK_DATE,CHK_USER,COMM,CREA_DATE,CREA_USER,BILL_NO,REMARK,TIME_STAMP) '
    + 'VALUES(:TENANT_ID,:RECV_ID,:SHOP_ID,:GLIDE_NO,:ACCOUNT_ID,:PAYM_ID,:RECV_MNY,:CLIENT_ID,:ITEM_ID,:RECV_DATE,:RECV_USER,:CHK_DATE,:CHK_USER,''00'','+GetSysDateFormat(iDbType)+',:CREA_USER,:BILL_NO,:REMARK,'+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
