...
begin
  if (Params.FindParam('SyncFlag')=nil) or (Params.FindParam('SyncFlag').asInteger=0) then
     ...
     if (FieldbyName('GLIDE_NO').AsString='') or (Pos('新',FieldbyName('GLIDE_NO').AsString)>0) then
        FieldbyName('GLIDE_NO').AsString := trimright(FieldbyName('SHOP_ID').AsString,4)+GetSequence(AGlobal,'GNO_5_'+FieldbyName('SHOP_ID').AsString,FieldbyName('TENANT_ID').AsString,formatDatetime('YYMMDD',now()),5);
     end;
  result := true;
end;
   ...
   if (Params.FindParam('SyncFlag')=nil) or (Params.FindParam('SyncFlag').asInteger=0) then
      begin
        ...
        if ((FieldbyName('PAYM_ID').AsString = 'A')
           ...
           and
           (FieldbyName('RECV_FLAG').asString='0')
        then  //已经结账不能操作现金收款
        begin
...
begin
  ...
  'select A.RECV_ID,A.TENANT_ID,A.SHOP_ID,A.DEPT_ID,A.RECV_FLAG,A.GLIDE_NO,A.ACCOUNT_ID,A.PAYM_ID,A.BANK_ID,A.BANK_CODE,A.CLIENT_ID,B.ACCT_NAME as ACCOUNT_ID_TEXT,A.ITEM_ID,'+
  ...
  'left outer join VIW_CUSTOMER e on je.TENANT_ID=e.TENANT_ID and je.CLIENT_ID=e.CLIENT_ID ) jf '+
  'left outer join CA_DEPT_INFO f on jf.TENANT_ID=f.TENANT_ID and jf.DEPT_ID=f.DEPT_ID ';
  IsSQLUpdate := True;
  Str := 'insert into ACC_RECVORDER(TENANT_ID,RECV_ID,SHOP_ID,DEPT_ID,RECV_FLAG,GLIDE_NO,ACCOUNT_ID,PAYM_ID,BANK_ID,BANK_CODE,RECV_MNY,CLIENT_ID,ITEM_ID,RECV_DATE,RECV_USER,CHK_DATE,CHK_USER,COMM,CREA_DATE,CREA_USER,BILL_NO,REMARK,TIME_STAMP) '
    + 'VALUES(:TENANT_ID,:RECV_ID,:SHOP_ID,:DEPT_ID,:RECV_FLAG,:GLIDE_NO,:ACCOUNT_ID,:PAYM_ID,:BANK_ID,:BANK_CODE,:RECV_MNY,:CLIENT_ID,:ITEM_ID,:RECV_DATE,:RECV_USER,:CHK_DATE,:CHK_USER,''00'','+GetSysDateFormat(iDbType)+',:CREA_USER,:BILL_NO,:REMARK,'+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  Str := 'update ACC_RECVORDER set TENANT_ID=:TENANT_ID,RECV_ID=:RECV_ID,SHOP_ID=:SHOP_ID,DEPT_ID=:DEPT_ID,RECV_FLAG=:RECV_FLAG,GLIDE_NO=:GLIDE_NO,ACCOUNT_ID=:ACCOUNT_ID,PAYM_ID=:PAYM_ID,BANK_ID=:BANK_ID,BANK_CODE=:BANK_CODE,CLIENT_ID=:CLIENT_ID,ITEM_ID=:ITEM_ID,'+
