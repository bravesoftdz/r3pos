...
begin
  ...
  begin
     AGlobal.ExecSQL(
       ...
       + 'VALUES('''+newid(FieldbyName('SHOP_ID').AsString)+''',:TENANT_ID,:SHOP_ID,:CLIENT_ID,'''+'预收款:【订单号'+FieldbyName('GLIDE_NO').AsString+'】'+''',3,:ADVA_MNY,0,0,:ADVA_MNY,:INDE_DATE,null,:INDE_ID,:CREA_DATE,:CREA_USER,''00'','+GetTimeStamp(iDbType)+')'
    ,self);
  end;
  result := true;
end;
...
begin
  ...
  SelectSQL.Text :=
               ...
               '(select TENANT_ID,SHOP_ID,INDE_ID,GLIDE_NO,INDE_DATE,PLAN_DATE,ADVA_MNY,LINKMAN,TELEPHONE,SEND_ADDR,SALES_STYLE,CLIENT_ID,GUIDE_USER,CHK_DATE,CHK_USER,FIG_ID,REMARK,INVOICE_FLAG,TAX_RATE,COMM,CREA_DATE,CREA_USER,'+
  ...
  IsSQLUpdate := True;
  Str :=
      'insert into SAL_INDENTORDER(TENANT_ID,SHOP_ID,INDE_ID,GLIDE_NO,INDE_DATE,PLAN_DATE,ADVA_MNY,LINKMAN,TELEPHONE,SEND_ADDR,SALES_STYLE,CLIENT_ID,GUIDE_USER,CHK_DATE,CHK_USER,INDE_AMT,'+
      'INDE_MNY,FIG_ID,REMARK,INVOICE_FLAG,TAX_RATE,COMM,CREA_DATE,CREA_USER,TIME_STAMP) '
    + 'VALUES(:TENANT_ID,:SHOP_ID,:INDE_ID,:GLIDE_NO,:INDE_DATE,:PLAN_DATE,:ADVA_MNY,:LINKMAN,:TELEPHONE,:SEND_ADDR,:SALES_STYLE,:CLIENT_ID,:GUIDE_USER,:CHK_DATE,:CHK_USER,:INDE_AMT,:INDE_MNY,:FIG_ID,:REMARK,:INVOICE_FLAG,:TAX_RATE,''00'',:CREA_DATE,:CREA_USER,'+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  Str := 'update SAL_INDENTORDER set TENANT_ID=:TENANT_ID,SHOP_ID=:SHOP_ID,INDE_ID=:INDE_ID,GLIDE_NO=:GLIDE_NO,INDE_DATE=:INDE_DATE,PLAN_DATE=:PLAN_DATE,ADVA_MNY=:ADVA_MNY,CLIENT_ID=:CLIENT_ID,INDE_AMT=:INDE_AMT,INDE_MNY=:INDE_MNY,'+
         'GUIDE_USER=:GUIDE_USER,CHK_DATE=:CHK_DATE,CHK_USER=:CHK_USER,LINKMAN=:LINKMAN,TELEPHONE=:TELEPHONE,SEND_ADDR=:SEND_ADDR,SALES_STYLE=:SALES_STYLE,FIG_ID=:FIG_ID,REMARK=:REMARK,INVOICE_FLAG=:INVOICE_FLAG,TAX_RATE=:TAX_RATE,'
...
begin
  inherited;
  case iDbType of
  0,3:SelectSQL.Text := 'select top 1 INDE_ID from SAL_INDENTORDER where TENANT_ID=:TENANT_ID and CREA_USER=:CREA_USER and GLIDE_NO>:GLIDE_NO order by GLIDE_NO';
  4:SelectSQL.Text := 'select * from (select INDE_ID from SAL_INDENTORDER where TENANT_ID=:TENANT_ID and CREA_USER=:CREA_USER and GLIDE_NO>:GLIDE_NO order by GLIDE_NO) tp fetch first 1 rows only';
  5:SelectSQL.Text := 'select INDE_ID from SAL_INDENTORDER where TENANT_ID=:TENANT_ID and CREA_USER=:CREA_USER and GLIDE_NO>:GLIDE_NO order by GLIDE_NO limit 1';
  end;
end;
...
begin
  try
    Str := 'update SAL_INDENTORDER set CHK_DATE='''+Params.FindParam('CHK_DATE').asString+''',CHK_USER='''+Params.FindParam('CHK_USER').asString+''' where TENANT_ID='+Params.FindParam('TENANT_ID').asString +' and INDE_ID='''+Params.FindParam('INDE_ID').asString+''' and CHK_DATE IS NULL';
...
begin
   try
    Str := 'update SAL_INDENTORDER set CHK_DATE=null,CHK_USER=null where TENANT_ID='+Params.FindParam('TENANT_ID').asString +' and INDE_ID='''+Params.FindParam('INDE_ID').asString+''' and CHK_DATE IS NOT NULL';
    n := AGlobal.ExecSQL(Str);
    if n=0 then
