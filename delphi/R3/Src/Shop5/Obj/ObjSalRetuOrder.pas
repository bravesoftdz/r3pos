...
begin
  ...
  if FieldbyName('SALES_MNY').AsOldFloat <> 0 then
     begin
       ...
       AGlobal.ExecSQL('delete from RCK_RECVABLE_INFO where TENANT_ID=:OLD_TENANT_ID and SALES_ID=:OLD_SALES_ID',self);
     end;
{
  ...
  begin
     ...
     begin
       ...
       AGlobal.ExecSQL(
         ...
         + 'VALUES('''+newid(FieldbyName('SHOP_ID').asString)+''',:TENANT_ID,:SHOP_ID,:CLIENT_ID,'''+'退货退款:【单号'+FieldbyName('GLIDE_NO').AsString+'】'+''',2,:PAY_D,0,-:ADVA_MNY,-:PAY_D + :ADVA_MNY,:SALES_DATE,null,:SALES_ID,:CREA_DATE,:CREA_USER,''00'','+GetTimeStamp(iDbType)+')'
      ,self);
     end;
     {
...
begin
  ...
  SelectSQL.Text :=
               ...
               '(select TENANT_ID,SHOP_ID,SALES_ID,GLIDE_NO,SALES_DATE,SALES_TYPE,LINKMAN,TELEPHONE,SEND_ADDR,PLAN_DATE,CLIENT_ID,GUIDE_USER,CHK_DATE,CHK_USER,FROM_ID,FIG_ID,-SALE_AMT as SALE_AMT,-SALE_MNY as SALE_MNY,CASH_MNY,PAY_ZERO,-PAY_DIBS as PAY_DIBS,'+
  ...
  Str := 'insert into SAL_SALESORDER(TENANT_ID,SHOP_ID,SALES_ID,GLIDE_NO,SALES_DATE,SALES_TYPE,IC_CARDNO,UNION_ID,PLAN_DATE,CLIENT_ID,GUIDE_USER,CHK_DATE,CHK_USER,FROM_ID,FIG_ID,SALE_AMT,SALE_MNY,CASH_MNY,PAY_ZERO,PAY_DIBS,ADVA_MNY,PAY_A,PAY_B,PAY_C,'+
      'PAY_D,PAY_E,PAY_F,PAY_G,PAY_H,PAY_I,PAY_J,INTEGRAL,REMARK,INVOICE_FLAG,TAX_RATE,COMM,CREA_DATE,CREA_USER,TIME_STAMP,LINKMAN,TELEPHONE,SEND_ADDR,SALES_STYLE) '
    + 'VALUES(:TENANT_ID,:SHOP_ID,:SALES_ID,:GLIDE_NO,:SALES_DATE,:SALES_TYPE,:IC_CARDNO,:UNION_ID,:PLAN_DATE,:CLIENT_ID,:GUIDE_USER,:CHK_DATE,:CHK_USER,:FROM_ID,'+
      ':FIG_ID,-:SALE_AMT,-:SALE_MNY,:CASH_MNY,:PAY_ZERO,-:PAY_DIBS,-:ADVA_MNY,-:PAY_A,-:PAY_B,-:PAY_C,-:PAY_D,'+
      '-:PAY_E,-:PAY_F,-:PAY_G,-:PAY_H,-:PAY_I,-:PAY_J,-:INTEGRAL,:REMARK,:INVOICE_FLAG,:TAX_RATE,''00'','+GetSysDateFormat(iDbType)+',:CREA_USER,'+GetTimeStamp(iDbType)+',:LINKMAN,:TELEPHONE,:SEND_ADDR,:SALES_STYLE)';
  InsertSQL.Text := Str;
