...
type
  ...
  //采购订单手工结案
  TStkIndentOrderHandClosed=class(TZProcFactory)
  public
    function Execute(AGlobal:IdbHelp;Params:TftParamList):Boolean;override;
  end;
...
begin
  ...
  SelectSQL.Text := 'select j.TENANT_ID,j.SHOP_ID,j.INDE_ID,j.SEQNO,j.GODS_ID,j.PROPERTY_01,j.PROPERTY_02,j.BATCH_NO,j.LOCUS_NO,j.BOM_ID,j.UNIT_ID,j.AMOUNT,j.ORG_PRICE,j.IS_PRESENT,j.APRICE,j.AMONEY,j.AGIO_RATE,j.AGIO_MONEY,j.CALC_AMOUNT,j.CALC_MONEY,'+
               'case when j.CALC_AMOUNT=0 then 0 else j.FNSH_AMOUNT / (cast(j.CALC_AMOUNT/(j.AMOUNT*1.000) as decimal(18,3))*1.000) end as FNSH_AMOUNT,j.BOND_MNY,'+
  ...
  IsSQLUpdate := True;
  Str := 'insert into STK_INDENTDATA(TENANT_ID,SHOP_ID,SEQNO,INDE_ID,GODS_ID,BATCH_NO,LOCUS_NO,BOM_ID,PROPERTY_01,PROPERTY_02,UNIT_ID,AMOUNT,ORG_PRICE,IS_PRESENT,APRICE,AMONEY,AGIO_RATE,AGIO_MONEY,CALC_AMOUNT,CALC_MONEY,BOND_MNY,REMARK) '
    + 'VALUES(:TENANT_ID,:SHOP_ID,:SEQNO,:INDE_ID,:GODS_ID,:BATCH_NO,:LOCUS_NO,:BOM_ID,:PROPERTY_01,:PROPERTY_02,:UNIT_ID,:AMOUNT,:ORG_PRICE,:IS_PRESENT,:APRICE,:AMONEY,:AGIO_RATE,:AGIO_MONEY,:CALC_AMOUNT,:CALC_MONEY,:BOND_MNY,:REMARK)';
  InsertSQL.Text := Str;
  Str := 'update STK_INDENTDATA set TENANT_ID=:TENANT_ID,SHOP_ID=:SHOP_ID,SEQNO=:SEQNO,INDE_ID=:INDE_ID,GODS_ID=:GODS_ID,BATCH_NO=:BATCH_NO,PROPERTY_01=:PROPERTY_01,PROPERTY_02=:PROPERTY_02,UNIT_ID=:UNIT_ID,AMOUNT=:AMOUNT,ORG_PRICE=:ORG_PRICE,'+
         'BOND_MNY=:BOND_MNY,IS_PRESENT=:IS_PRESENT,APRICE=:APRICE,AMONEY=:AMONEY,AGIO_RATE=:AGIO_RATE,AGIO_MONEY=:AGIO_MONEY,CALC_AMOUNT=:CALC_AMOUNT,CALC_MONEY=:CALC_MONEY,REMARK=:REMARK '
...
begin
  ...
  if FieldByName('BOND_MNY').AsOldFloat <> 0 then
  begin
     rs := TZQuery.Create(nil);
     try
       rs.SQL.Text := 'select PAYM_MNY from ACC_PAYABLE_INFO where TENANT_ID='+FieldByName('TENANT_ID').AsOldString+' and STOCK_ID='+QuotedStr(FieldByName('INDE_ID').AsOldString)+' and ABLE_TYPE=''8'' ';
       AGlobal.Open(rs);
       if rs.Fields[0].AsFloat <> 0 then Raise Exception.Create('已经付款的订货单不能修改...');
     finally
       rs.Free;
     end;
     AGlobal.ExecSQL('delete from ACC_PAYABLE_INFO where TENANT_ID='+FieldByName('TENANT_ID').AsOldString+' and STOCK_ID='+QuotedStr(FieldByName('INDE_ID').AsOldString)+' and ABLE_TYPE=''8'' ');
  end;
  result := true;
end;
...
begin
  ...
  begin
     AGlobal.ExecSQL('insert into ACC_PAYABLE_INFO(ABLE_ID,TENANT_ID,SHOP_ID,DEPT_ID,CLIENT_ID,ACCT_INFO,ABLE_TYPE,ACCT_MNY,PAYM_MNY,REVE_MNY,RECK_MNY,ABLE_DATE,STOCK_ID,CREA_DATE,CREA_USER,COMM,TIME_STAMP) '
       + 'VALUES('''+newid(FieldbyName('SHOP_ID').AsString)+''',:TENANT_ID,:SHOP_ID,:DEPT_ID,:CLIENT_ID,'''+'滚存保证金【订单号'+FieldbyName('GLIDE_NO').AsString+'】'+''',''8'',:BOND_MNY,0,0,:BOND_MNY,:INDE_DATE,:INDE_ID,:CREA_DATE,:CREA_USER,''00'','+GetTimeStamp(iDbType)+')',Self);
  end;
  result := true;
end;
...
begin
  ...
  SelectSQL.Text :=
               ...
               'TIME_STAMP,INDE_AMT,BOND_MNY,INDE_MNY,STKBILL_STATUS from STK_INDENTORDER where TENANT_ID=:TENANT_ID and INDE_ID=:INDE_ID) jb '+
  ...
  IsSQLUpdate := True;
  Str := 'insert into STK_INDENTORDER(TENANT_ID,SHOP_ID,DEPT_ID,INDE_ID,GLIDE_NO,INDE_DATE,GUIDE_USER,CLIENT_ID,CHK_DATE,CHK_USER,PLAN_DATE,'+
      'BOND_MNY,ADVA_MNY,FIG_ID,INDE_AMT,INDE_MNY,INVOICE_FLAG,TAX_RATE,REMARK,STKBILL_STATUS,COMM,CREA_DATE,CREA_USER,TIME_STAMP) '+
      'VALUES(:TENANT_ID,:SHOP_ID,:DEPT_ID,:INDE_ID,:GLIDE_NO,:INDE_DATE,:GUIDE_USER,:CLIENT_ID,:CHK_DATE,:CHK_USER,:PLAN_DATE,'+
      ':BOND_MNY,:ADVA_MNY,:FIG_ID,:INDE_AMT,:INDE_MNY,:INVOICE_FLAG,:TAX_RATE,:REMARK,0,''00'',:CREA_DATE,:CREA_USER,'+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  Str := 'update STK_INDENTORDER set TENANT_ID=:TENANT_ID,SHOP_ID=:SHOP_ID,DEPT_ID=:DEPT_ID,INDE_ID=:INDE_ID,GLIDE_NO=:GLIDE_NO,INDE_DATE=:INDE_DATE,ADVA_MNY=:ADVA_MNY,GUIDE_USER=:GUIDE_USER,CLIENT_ID=:CLIENT_ID,CHK_DATE=:CHK_DATE,CHK_USER=:CHK_USER,'+
         'BOND_MNY=:BOND_MNY,PLAN_DATE=:PLAN_DATE,FIG_ID=:FIG_ID,INDE_AMT=:INDE_AMT,INDE_MNY=:INDE_MNY,INVOICE_FLAG=:INVOICE_FLAG,TAX_RATE=:TAX_RATE,REMARK=:REMARK,'
...
{ TStkIndentOrderHandClosed }

function TStkIndentOrderHandClosed.Execute(AGlobal: IdbHelp; Params: TftParamList): Boolean;
var
  rs:TZQuery;
  r:Currency;
  UpSQL:string;
begin
  ...
  try
    UpSQL:=
      'update STK_INDENTORDER '+
      ' set STKBILL_STATUS=2,COMM='+GetCommStr(AGlobal.iDbType)+',TIME_STAMP='+GetTimeStamp(AGlobal.iDbType)+
    ...
    begin
      UpSQL:='update STK_STOCKORDER '+
            ' set ADVA_MNY=ADVA_MNY+ '+formatFloat('#0.00',r)+',COMM='+GetCommStr(AGlobal.iDbType)+',TIME_STAMP='+GetTimeStamp(AGlobal.iDbType)+
            ' where TENANT_ID='+Params.ParamByName('TENANT_ID').AsString+' and STOCK_ID='''+Params.ParamByName('BILL_ID').AsString+''' ';
      AGlobal.ExecSQL(UpSQL);
      UpSQL:='update ACC_PAYABLE_INFO '+
...
initialization
  ...
  RegisterClass(TStkIndentOrderHandClosed);
...
finalization
  ...
  UnRegisterClass(TStkIndentOrderHandClosed);
end.
