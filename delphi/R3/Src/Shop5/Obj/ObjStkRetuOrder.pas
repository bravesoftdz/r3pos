...
begin
  ...
  begin
     ...
     FieldbyName('GLIDE_NO').asString := GetSequence(AGlobal,'GNO_2_'+FieldbyName('SHOP_ID').AsString,FieldbyName('TENANT_ID').AsString,formatDatetime('YYMMDD',now()),5);
...
begin
  ...
  SelectSQL.Text :=
               ...
               '-STOCK_MNY as STOCK_MNY,-STOCK_AMT as STOCK_AMT,-ADVA_MNY as ADVA_MNY,TAX_RATE*100.0 as TAX_RATE,REMARK,COMM,CREA_DATE,CREA_USER,TIME_STAMP from STK_STOCKORDER where TENANT_ID=:TENANT_ID and STOCK_ID=:STOCK_ID) jb '+
  ...
  InsertSQL.Text := str;
  Str := 'update STK_STOCKORDER set TENANT_ID=:TENANT_ID,SHOP_ID=:SHOP_ID,STOCK_ID=:STOCK_ID,GLIDE_NO=:GLIDE_NO,STOCK_TYPE=:STOCK_TYPE,STOCK_DATE=:STOCK_DATE,GUIDE_USER=:GUIDE_USER,CLIENT_ID=:CLIENT_ID,CHK_DATE=:CHK_DATE,CHK_USER=:CHK_USER,'+
         'FROM_ID=:FROM_ID,FIG_ID=:FIG_ID,INVOICE_FLAG=:INVOICE_FLAG,TAX_RATE=:TAX_RATE / 100.0,STOCK_MNY=-:STOCK_MNY,STOCK_AMT=-:STOCK_AMT,ADVA_MNY=-:ADVA_MNY,REMARK=:REMARK,CREA_DATE=:CREA_DATE,CREA_USER=:CREA_USER,'
...
begin
  try
    Str := 'update STK_STOCKORDER set CHK_DATE='''+Params.FindParam('CHK_DATE').asString+''',CHK_USER='''+Params.FindParam('CHK_USER').asString+''' where TENANT_ID='+Params.FindParam('TENANT_ID').asString+' and STOCK_ID='''+Params.FindParam('STOCK_ID').asString+''' and CHK_DATE IS NULL';
...
begin
   try
    Str := 'update STK_STOCKORDER set CHK_DATE=null,CHK_USER=null where TENANT_ID='+Params.FindParam('TENANT_ID').asString+' and STOCK_ID='''+Params.FindParam('STOCK_ID').asString+''' and CHK_DATE IS NOT NULL';
    n := AGlobal.ExecSQL(Str);
    if n=0 then
