...
begin
  ...
  AGlobal.ExecSQL(Str,self);
  if FieldbyName('SHORT_TENANT_NAME').AsString='' then
    Str := 'insert into CA_SHOP_INFO(SHOP_ID,LICENSE_CODE,XSM_CODE,XSM_PSWD,SHOP_NAME,SHOP_SPELL,SHOP_TYPE,TENANT_ID,REGION_ID,SEQ_NO,LINKMAN,'+
    'TELEPHONE,ADDRESS,POSTALCODE,FAXES,COMM,TIME_STAMP) values('+shopid+',:LICENSE_CODE,:XSM_CODE,:XSM_PSWD,:TENANT_NAME,:TENANT_SPELL,'+
    '''CA73FC48-985B-4EE7-9BDA-B7D638BD25F6'',:TENANT_ID,:REGION_ID,1001,:LINKMAN,:TELEPHONE,:ADDRESS,:POSTALCODE,:FAXES,''00'',5497000)'
  else
    Str := 'insert into CA_SHOP_INFO(SHOP_ID,LICENSE_CODE,XSM_CODE,XSM_PSWD,SHOP_NAME,SHOP_SPELL,SHOP_TYPE,TENANT_ID,REGION_ID,SEQ_NO,LINKMAN,'+
    'TELEPHONE,ADDRESS,POSTALCODE,FAXES,COMM,TIME_STAMP) values('+shopid+',:LICENSE_CODE,:XSM_CODE,:XSM_PSWD,:SHORT_TENANT_NAME,:TENANT_SPELL,'+
    '''CA73FC48-985B-4EE7-9BDA-B7D638BD25F6'',:TENANT_ID,:REGION_ID,1001,:LINKMAN,:TELEPHONE,:ADDRESS,:POSTALCODE,:FAXES,''00'',5497000)';
  AGlobal.ExecSQL(Str,Self);
...
begin
  ...
  SelectSQL.Text := 'Select TENANT_ID,LICENSE_CODE,LOGIN_NAME,TENANT_NAME,TENANT_TYPE,SHORT_TENANT_NAME,TENANT_SPELL,LEGAL_REPR,LINKMAN,TELEPHONE,FAXES,'+
                    'HOMEPAGE,QQ,MSN,ADDRESS,POSTALCODE,REMARK,PASSWRD,REGION_ID,SRVR_ID,PROD_ID,AUDIT_STATUS,''                              '' as XSM_CODE,''                              '' as XSM_PSWD From CA_TENANT where TENANT_ID=:TENANT_ID';
  IsSQLUpdate := True;
...
try
  ...
  'values(:TENANT_ID,''#'',''普通客户'',''PTKH'',0,0,0,0,0,0,1,''00'',5497000)';
  AGlobal.ExecSQL(Str,Params);
  Str := 'insert into PUB_PRICEGRADE(TENANT_ID,PRICE_ID,PRICE_NAME,PRICE_SPELL,INTEGRAL,INTE_TYPE,INTE_AMOUNT,MINIMUM_PERCENT,AGIO_TYPE,AGIO_PERCENT,SEQ_NO,COMM,TIME_STAMP)'+
  'values(:TENANT_ID,''332E5CE8-D1FC-4D53-84C3-2A1EB07EF8AA'',''银卡客户'',''YKKH'',0,0,0,0,0,0,1,''00'',5497000)';
  AGlobal.ExecSQL(Str,Params);
  Str := 'insert into PUB_PRICEGRADE(TENANT_ID,PRICE_ID,PRICE_NAME,PRICE_SPELL,INTEGRAL,INTE_TYPE,INTE_AMOUNT,MINIMUM_PERCENT,AGIO_TYPE,AGIO_PERCENT,SEQ_NO,COMM,TIME_STAMP)'+
  'values(:TENANT_ID,''26AB5597-5AA5-49BC-8CBF-7A332CB1B6FA'',''金卡客户'',''JKKH'',0,0,0,0,0,0,1,''00'',5497000)';
  AGlobal.ExecSQL(Str,Params);
  Str := 'insert into PUB_PRICEGRADE(TENANT_ID,PRICE_ID,PRICE_NAME,PRICE_SPELL,INTEGRAL,INTE_TYPE,INTE_AMOUNT,MINIMUM_PERCENT,AGIO_TYPE,AGIO_PERCENT,SEQ_NO,COMM,TIME_STAMP)'+
  'values(:TENANT_ID,''4433497A-0A2E-4D72-BE0B-723850A6EFE8'',''钻石客户'',''ZSKH'',0,0,0,0,0,0,1,''00'',5497000)';
  AGlobal.ExecSQL(Str,Params);
  end;
  Result := True;
  AGlobal.CommitTrans;
