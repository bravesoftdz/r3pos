...
begin
  ...
  SelectSQL.Text := 'select USER_ID,ACCOUNT,ENCODE,USER_NAME,USER_SPELL,PASS_WRD,SHOP_ID,DEPT_ID,DUTY_IDS,DUTY_NAMES,ROLE_IDS,ROLE_NAMES,TENANT_ID,SEX,MOBILE,OFFI_TELE,FAMI_TELE,EMAIL,QQ,MSN,'+
                    'ID_NUMBER,IDN_TYPE,FAMI_ADDR,POSTALCODE,WORK_DATE,DIMI_DATE,REMARK,COMM,TIME_STAMP from CA_USERS where USER_ID=:USER_ID and TENANT_ID=:TENANT_ID ORDER BY USER_ID';
  IsSQLUpdate := True;
  Str := 'insert into CA_USERS(USER_ID,ACCOUNT,USER_NAME,USER_SPELL,SHOP_ID,DEPT_ID,DUTY_IDS,DUTY_NAMES,TENANT_ID,SEX,MOBILE,OFFI_TELE,FAMI_TELE,'+
         ...
         ':USER_SPELL,:SHOP_ID,:DEPT_ID,:DUTY_IDS,:DUTY_NAMES,:TENANT_ID,:SEX,:MOBILE,:OFFI_TELE,:FAMI_TELE,:EMAIL,:QQ,:MSN,:ID_NUMBER,:IDN_TYPE,'+
         ':FAMI_ADDR,:POSTALCODE,:WORK_DATE,:DIMI_DATE,:REMARK,:COMM,:TIME_STAMP,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Add( Str);
  Str := 'update CA_USERS set USER_ID=:USER_ID,ACCOUNT=:ACCOUNT,USER_NAME=:USER_NAME,USER_SPELL=:USER_SPELL,SHOP_ID=:SHOP_ID,DEPT_ID=:DEPT_ID,DUTY_IDS=:DUTY_IDS,'+
         ...
         ',TIME_STAMP='+GetTimeStamp(iDbType)+' where USER_ID=:OLD_USER_ID';
  UpdateSQL.Add( Str);
  Str := 'update CA_USERS set COMM=''02'',TIME_STAMP='+GetTimeStamp(iDbType)+' where USER_ID=:OLD_USER_ID';
  DeleteSQL.Add( Str);
end;
...
begin
  ...
  try
    ...
    try
      ...
      tmp.Close;
      tmp.SQL.Add('select * from SAL_SALESORDER where GUIDE_USER='''+Params.ParamByName('USER_ID').asString+''' or OPER_USER='''+Params.ParamByName('USER_ID').asString+'''');
      AGlobal.Open(tmp);
      if tmp.RecordCount>0 then Raise Exception.Create('此用户在销售单据中有使用,不能删除!');
    finally
      tmp.Free;
    end;
    AGlobal.ExecSQL('update CA_USERS set  COMM=''02'',TIME_STAMP='+GetTimeStamp(AGlobal.iDbType)+' where USER_ID='+Params.ParamByName('USER_ID').asString);
    AGlobal.CommitTrans;
    Result:=True;
  except
    on E:Exception do
