...
begin
  ...
  Locked := false;
  SelectSQL.Text :=
  ' select A.TENANT_ID,A.VOUCHER_ID,A.SHOP_ID,A.VOUCHER_TYPE,A.INTO_DATE,C.SHOP_NAME as SHOP_ID_TEXT,A.DEPT_ID,B.DEPT_NAME as DEPT_ID_TEXT,A.VOUCHER_PRC,'+
  'A.CREA_DATE,A.CREA_USER,D.USER_NAME as CREA_USER_TEXT,A.INTO_USER,E.USER_NAME as INTO_USER_TEXT,A.VAILD_DATE,A.VUCH_NAME,A.REMARK,A.PRINT_TIMES,A.COMM '+
  ...
  IsSQLUpdate := True;
  Str := ' insert into SAL_VOUCHERORDER(TENANT_ID,VOUCHER_ID,SHOP_ID,DEPT_ID,VOUCHER_TYPE,INTO_DATE,INTO_USER,CREA_DATE,CREA_USER,VOUCHER_PRC,VAILD_DATE,VUCH_NAME,REMARK,COMM,TIME_STAMP) '+
  ' VALUES(:TENANT_ID,:VOUCHER_ID,:SHOP_ID,:DEPT_ID,:VOUCHER_TYPE,:INTO_DATE,:INTO_USER,:CREA_DATE,:CREA_USER,:VOUCHER_PRC,:VAILD_DATE,:VUCH_NAME,:REMARK,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  Str := ' update SAL_VOUCHERORDER set TENANT_ID=:TENANT_ID,SHOP_ID=:SHOP_ID,DEPT_ID=:DEPT_ID,VOUCHER_TYPE=:VOUCHER_TYPE,INTO_DATE=:INTO_DATE,'+
         'INTO_USER=:INTO_USER,CREA_DATE=:CREA_DATE,CREA_USER=:CREA_USER,VOUCHER_PRC=:VOUCHER_PRC,VAILD_DATE=:VAILD_DATE,VUCH_NAME=:VUCH_NAME,REMARK=:REMARK,'+
...
begin
  inherited;
  SelectSQL.Text :=
  'select TENANT_ID,VOUCHER_ID,SEQNO,BARCODE,VOUCHER_TYPE,VOUCHER_PRC,VOUCHER_STATUS,VAILD_DATE,CLIENT_ID,COMM '+
  ' from SAL_VOUCHERDATA where TENANT_ID=:TENANT_ID and VOUCHER_ID=:VOUCHER_ID order by BARCODE ';
  IsSQLUpdate := True;
  Str := 'insert into SAL_VOUCHERDATA(TENANT_ID,VOUCHER_ID,SEQNO,BARCODE,VOUCHER_TYPE,VOUCHER_PRC,VOUCHER_STATUS,VAILD_DATE,CLIENT_ID,COMM,TIME_STAMP) '
    + 'values(:TENANT_ID,:VOUCHER_ID,:SEQNO,:BARCODE,:VOUCHER_TYPE,:VOUCHER_PRC,:VOUCHER_STATUS,:VAILD_DATE,:CLIENT_ID,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  Str := 'update SAL_VOUCHERDATA set TENANT_ID=:TENANT_ID,VOUCHER_ID=:VOUCHER_ID,SEQNO=:SEQNO,BARCODE=:BARCODE,VOUCHER_TYPE=:VOUCHER_TYPE,'+
