...
as  
SELECT TENANT_ID,SHOP_ID,trim(char(CLSE_DATE)) as GLIDE_NO,CLSE_DATE AS RECV_DATE, CREA_USER, CHK_DATE, CLSE_DATE AS ACC_DATE,'1' as RECV_TYPE,'1'as BILL_TYPE, 
       COALESCE(PAY_A,0)+ COALESCE(PAY_B,0)+COALESCE(PAY_C,0)+COALESCE(PAY_E,0)+COALESCE(PAY_F,0)+COALESCE(PAY_G,0)+COALESCE(PAY_H,0)+COALESCE(PAY_I,0)+COALESCE(PAY_J, 0) AS ACCT_MNY,
       COALESCE(PAY_A,0)+ COALESCE(PAY_B,0)+COALESCE(PAY_C,0)+COALESCE(PAY_E,0)+COALESCE(PAY_F,0)+COALESCE(PAY_G,0)+COALESCE(PAY_H,0)+COALESCE(PAY_I,0)+COALESCE(PAY_J, 0) as RECV_MNY,
       0 as  RECK_MNY,0 as OVERDAYS,'零售结帐单' as ACCT_INFO,'' as CLIENT_ID 
FROM  ACC_CLOSE_FORDAY
UNION ALL
SELECT TENANT_ID,SHOP_ID,IC_CARDNO as GLIDE_NO,CREA_DATE AS RECV_DATE, CREA_USER, CHK_DATE, CREA_DATE AS ACC_DATE,'1' as RECV_TYPE,'2'as BILL_TYPE,  
       COALESCE(PAY_A,0)+COALESCE(PAY_B,0)+COALESCE(PAY_C,0)+COALESCE(PAY_E,0)+COALESCE(PAY_F,0)+COALESCE(PAY_G,0)+COALESCE(PAY_H,0)+COALESCE(PAY_I,0)+COALESCE(PAY_J,0) AS ACCT_MNY,
       COALESCE(PAY_A,0)+COALESCE(PAY_B,0)+COALESCE(PAY_C,0)+COALESCE(PAY_E,0)+COALESCE(PAY_F,0)+COALESCE(PAY_G,0)+COALESCE(PAY_H,0)+COALESCE(PAY_I,0)+COALESCE(PAY_J,0) AS RECV_MNY,
       0 as RECK_MNY,0 as OVERDAYS,'储值卡充值收款' as ACCT_INFO,IC_CARDNO as CLIENT_ID 
FROM SAL_IC_GLIDE
WHERE (IC_GLIDE_TYPE = '1')
UNION ALL
SELECT A.TENANT_ID,A.SHOP_ID,GLIDE_NO as GLIDE_NO,B.RECV_DATE, B.RECV_USER AS CREA_USER, B.CHK_DATE, C.ABLE_DATE AS ACC_DATE,A.RECV_TYPE as RECV_TYPE,'3'as BILL_TYPE,
       C.ACCT_MNY,A.RECV_MNY,C.RECK_MNY,
       (case when DAYS(to_date(char(RECV_DATE),'yyyy-mm-dd'))-DAYS(to_date(char(ABLE_DATE),'yyyy-mm-dd'))>0 then DAYS(to_date(char(RECV_DATE),'yyyy-mm-dd'))-DAYS(to_date(char(ABLE_DATE),'yyyy-mm-dd')) else 0 end) as OVERDAYS,
       C.ACCT_INFO as ACCT_INFO,B.CLIENT_ID as CLIENT_ID
FROM ACC_RECVDATA AS A 
     INNER JOIN ACC_RECVORDER AS B ON A.TENANT_ID = B.TENANT_ID AND A.RECV_ID = B.RECV_ID 
     INNER JOIN ACC_RECVABLE_INFO AS C ON A.TENANT_ID = C.TENANT_ID and A.ABLE_ID = C.ABLE_ID;


--供应商付款报表
CREATE VIEW VIW_PAYABLEDATA
as
select A.*,B.ABLE_DATE,B.ACCT_MNY,B.PAYM_MNY,B.RECK_MNY,B.ACCT_INFO,
 (case when DAYS(to_date(char(PAY_DATE),'yyyy-mm-dd'))-DAYS(to_date(char(ABLE_DATE),'yyyy-mm-dd'))>0 then DAYS(to_date(char(PAY_DATE),'yyyy-mm-dd'))-DAYS(to_date(char(ABLE_DATE),'yyyy-mm-dd')) else 0 end) as OVERDAYS  
from 
  VIW_PAYDATA A
  left outer join ACC_PAYABLE_INFO B 
