...
union all
select 6 as FLAG,A.TENANT_ID,A.SHOP_ID,B.ACCOUNT_ID,CLSE_DATE as CREA_DATE,case when CLSE_TYPE='1' then PAY_A else 0 end as IN_MNY,0 as OUT_MNY,0 as PAY_MNY,0 as RECV_MNY,case when CLSE_TYPE='1' then PAY_A else 0 end as POS_MNY,0 as TRN_IN_MNY,0 as TRN_OUT_MNY,case when CLSE_TYPE='2' then PAY_A else 0 end as PUSH_MNY,0 as IORO_IN_MNY,0 as IORO_OUT_MNY from ACC_CLOSE_FORDAY A,ACC_ACCOUNT_INFO B where A.TENANT_ID=B.TENANT_ID and A.SHOP_ID=B.SHOP_ID and B.PAYM_ID='A';
...
--(1)SQLITE库：
//收银结账，包抱没有缴款登记的 VIW_RECVALLDATA
...
as 
SELECT A.*,B.ABLE_DATE,B.ACCT_MNY,B.RECV_MNY as SETT_MNY,B.RECV_MNY,B.RECK_MNY,B.ACCT_INFO,B.CLIENT_ID,B.RECV_TYPE,B.SALES_ID,B.CREA_USER as ABLE_USER
FROM VIW_RECVDATA A
...
as
select A.*,B.ABLE_DATE,B.ACCT_MNY,B.PAYM_MNY as SETT_MNY,B.RECV_MNY,B.RECK_MNY,B.ACCT_INFO,B.CLIENT_ID,B.ABLE_TYPE,B.SALES_ID,B.CREA_USER as ABLE_USER 
