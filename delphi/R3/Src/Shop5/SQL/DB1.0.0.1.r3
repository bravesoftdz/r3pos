...
as
    SELECT 
      j1.*,j2.BARCODE,j2.BATCH_NO,j2.PROPERTY_01,j2.PROPERTY_02,j2.BARCODE_TYPE,j2.BARCODE_UNIT_ID
    FROM 
...
as
    ...
    FROM 
      VIW_GOODSPRICEEXT j1 LEFT JOIN 
      VIW_GOODSSORT j2 ON j1.TENANT_ID = j2.TENANT_ID AND j1.SORT_ID1 = j2.SORT_ID on j1.RELATION_ID=j2.RELATION_ID; 
...
--积分类型
insert into PUB_PARAMS(CODE_ID,CODE_NAME,TYPE_CODE,COMM,TIME_STAMP) values('1','赠送积分','INTEGRAL_FLAG','00',strftime('%s','now','localtime')-1293840000);
insert into PUB_PARAMS(CODE_ID,CODE_NAME,TYPE_CODE,COMM,TIME_STAMP) values('2','兑换礼券','INTEGRAL_FLAG','00',strftime('%s','now','localtime')-1293840000);
insert into PUB_PARAMS(CODE_ID,CODE_NAME,TYPE_CODE,COMM,TIME_STAMP) values('3','返还现金','INTEGRAL_FLAG','00',strftime('%s','now','localtime')-1293840000);
...
as
select 1 as FLAG,TENANT_ID,SHOP_ID,PAY_USER as CREA_USER,ACCOUNT_ID,PAY_DATE as CREA_DATE,0 as IN_MNY,PAY_MNY as OUT_MNY from VIW_PAYDATA
union all
select 2 as FLAG,TENANT_ID,SHOP_ID,RECV_USER as CREA_USER,ACCOUNT_ID,RECV_DATE as CREA_DATE,RECV_MNY as IN_MNY,0 as OUT_MNY from VIW_RECVDATA
union all
select 3 as FLAG,TENANT_ID,SHOP_ID,IORO_USER as CREA_USER,ACCOUNT_ID,IORO_DATE as CREA_DATE,IN_MONEY as IN_MNY,OUT_MONEY as OUT_MNY from VIW_IORODATA
union all
