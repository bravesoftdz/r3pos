...
create view VIW_ACCT_FORPAY_A
as
select 1 as FLAG,TENANT_ID,SHOP_ID,ACCOUNT_ID,PAY_DATE as CREA_DATE,0 as IN_MNY,PAY_MNY as OUT_MNY from VIW_PAYDATA where PAYM_ID='A'
union all
select 2 as FLAG,TENANT_ID,SHOP_ID,ACCOUNT_ID,RECV_DATE as CREA_DATE,RECV_MNY as IN_MNY,0 as OUT_MNY from VIW_RECVDATA where PAYM_ID='A'
union all
select 3 as FLAG,A.TENANT_ID,A.SHOP_ID,A.ACCOUNT_ID,A.IORO_DATE as CREA_DATE,A.IN_MONEY as IN_MNY,A.OUT_MONEY as OUT_MNY from VIW_IORODATA A,ACC_ACCOUNT_INFO B where A.TENANT_ID=B.TENANT_ID and A.ACCOUNT_ID=B.ACCOUNT_ID and B.PAYM_ID='A'
union all
select 4 as FLAG,A.TENANT_ID,A.SHOP_ID,A.IN_ACCOUNT_ID as ACCOUNT_ID,A.TRANS_DATE as CREA_DATE,A.TRANS_MNY as IN_MNY,0 as OUT_MNY from ACC_TRANSORDER A,ACC_ACCOUNT_INFO B where A.TENANT_ID=B.TENANT_ID and A.IN_ACCOUNT_ID=B.ACCOUNT_ID and B.PAYM_ID='A'
union all
select 4 as FLAG,A.TENANT_ID,A.SHOP_ID,A.OUT_ACCOUNT_ID as ACCOUNT_ID,A.TRANS_DATE as CREA_DATE,0 as IN_MNY,A.TRANS_MNY as OUT_MNY from ACC_TRANSORDER A,ACC_ACCOUNT_INFO B where A.TENANT_ID=B.TENANT_ID and A.OUT_ACCOUNT_ID=B.ACCOUNT_ID and B.PAYM_ID='A'
union all
select 5 as FLAG,A.TENANT_ID,A.SHOP_ID,B.ACCOUNT_ID,CLSE_DATE as CREA_DATE,PAY_A as IN_MNY,0 as OUT_MNY from ACC_CLOSE_FORDAY A,ACC_ACCOUNT_INFO B where A.TENANT_ID=B.TENANT_ID and A.SHOP_ID=B.SHOP_ID and B.PAYM_ID='A'
union all
select 6 as FLAG,A.TENANT_ID,A.SHOP_ID,B.ACCOUNT_ID,A.CREA_DATE,PAY_A as IN_MNY,0 as OUT_MNY from SAL_IC_GLIDE A,ACC_ACCOUNT_INFO B where A.TENANT_ID=B.TENANT_ID and A.SHOP_ID=B.SHOP_ID and B.PAYM_ID='A';
