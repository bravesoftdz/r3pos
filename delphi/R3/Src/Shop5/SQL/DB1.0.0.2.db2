...
CREATE TABLE MSC_MESSAGE(
    ...
    --信息类型
	MSG_CLASS varchar(36) NOT NULL,
    --发布日期
	ISSUE_DATE int NOT NULL,
    --发布企业
	ISSUE_TENANT_ID int NOT NULL,
    --信息来源
...
CREATE INDEX I_MMSG_TENANT_ID ON MSC_MESSAGE(TENANT_ID);
CREATE INDEX I_MMSG_TIME_STAMP ON MSC_MESSAGE(TENANT_ID,TIME_STAMP);
CREATE INDEX I_MMSG_ISSUE_DATE ON MSC_MESSAGE(TENANT_ID,ISSUE_DATE);
CREATE INDEX I_MMSG_MSG_CLASS ON MSC_MESSAGE(TENANT_ID,MSG_CLASS);

CREATE INDEX I_MMSG_LIST_MSG_ID ON MSC_MESSAGE_LIST(TENANT_ID,MSG_ID);
...
CREATE TABLE MSC_QUESTION(
    ...
    --问卷类型
	QUESTION_CLASS varchar(36) NOT NULL,
    --发布日期
	ISSUE_DATE int NOT NULL,
    --发布企业
	ISSUE_TENANT_ID int NOT NULL,
    ...
    --是否允许重复提交
	ANSWER_FLAG varchar(1) NOT NULL,
    --问题数
	QUESTION_ITEM_AMT int NOT NULL,
    --问卷说明
...
CREATE INDEX I_MQST_TENANT_ID ON MSC_QUESTION(TENANT_ID);
CREATE INDEX I_MQST_TIME_STAMP ON MSC_QUESTION(TENANT_ID,TIME_STAMP);
CREATE INDEX I_MQST_ISSUE_DATE ON MSC_QUESTION(TENANT_ID,ISSUE_DATE);
CREATE INDEX I_MQST_QUESTION_CLASS ON MSC_QUESTION(TENANT_ID,QUESTION_CLASS);
...
CREATE TABLE MSC_QUESTION_ITEM(
	...
	SEQ_NO int NOT NULL,
    --题目类型
	QUESTION_ITEM_TYPE varchar(1) NOT NULL,
    --题目内容
...
CREATE INDEX IX_MSC_QUESTION_ITEM_QUESTION_ID ON MSC_QUESTION_ITEM(TENANT_ID,QUESTION_ID);
...
CREATE TABLE MSC_INVEST_LIST(
	...
	SHOP_ID varchar (13) NOT NULL,
    --答题时间
	ANSWER_DATE varchar(19),
    --答题用户
	ANSWER_USER varchar(36),
    --答题时长
	ANSWER_USE_TIME int,
    --反馈状态<指是否向上级单位或第三方提交>
	QUESTION_FEEDBACK_STATUS int,
    --阅读标志
	QUESTION_ANSWER_STATUS int,
    --通讯标志 00 新增 01修改 02 删除 第一位为0待同步,1已同步
	COMM varchar(2) NOT NULL  DEFAULT '00',
    --更新时间 从2011-01-01开始的秒数  
	TIME_STAMP bigint NOT NULL,
...
CREATE INDEX I_MIST_QUESTION_ID ON MSC_INVEST_LIST(TENANT_ID,QUESTION_ID);
...
CREATE TABLE MSC_INVEST_ANSWER(
	...
	QUESTION_ITEM_ID char(36) NOT NULL,
	  --答案
	ANSWER_VALUE varchar(300) NOT NULL,
    --通讯标志 00 新增 01修改 02 删除 第一位为0待同步,1已同步
	COMM varchar(2) NOT NULL  DEFAULT '00',
    --更新时间 从2011-01-01开始的秒数  
	TIME_STAMP bigint NOT NULL,
...
CREATE INDEX I_MISA_QUESTION_ID ON MSC_INVEST_ANSWER(TENANT_ID,QUESTION_ID);
...
CREATE TABLE MSC_IMPEACH(
	...
	SHOP_ID varchar (13) NOT NULL,
    --向指定企业投诉
	IMPH_TENANT_ID int NOT NULL,
	  --投诉日期
	IMPH_DATE int NOT NULL,
	  --投诉时间
	CREA_DATE varchar(19),
	  --投诉用户
	CREA_USER varchar(36) NOT NULL,
	  --类型
	IMPEACH_CLASS varchar(36) NOT NULL,
	  --反馈状态<指是否向上级单位或第三方提交>
	IMPEACH_FEEDBACK_STATUS varchar(36) NOT NULL,
    --是否重复投拆 1重复 0否
	...
	CONTENT varchar(500) NOT NULL,
	  --投诉或建议对象<人>
	IMPEACH_USER varchar(300) NOT NULL,
	  --处理状态
	IMPEACH_PROC_STATUS varchar(36) NOT NULL,
	  --处理方法
	IMPEACH_PROC_INFO varchar(200),
	  --处理时间
	IMPEACH_PROC_DATE varchar(19),
    --通讯标志 00 新增 01修改 02 删除 第一位为0待同步,1已同步
	COMM varchar(2) NOT NULL  DEFAULT '00',
    --更新时间 从2011-01-01开始的秒数  
	TIME_STAMP bigint NOT NULL,
...
CREATE INDEX I_MIPH_TENANT_ID ON MSC_IMPEACH(TENANT_ID);
CREATE INDEX I_MIPH_TIME_STAMP ON MSC_IMPEACH(TENANT_ID,TIME_STAMP);
