--修正会员卡表的重复记录
delete from PUB_IC_INFO where UNION_ID='#' and not Exists(select * from PUB_CUSTOMER where TENANT_ID=PUB_IC_INFO.TENANT_ID and CUST_ID=PUB_IC_INFO.CLIENT_ID and CUST_CODE=PUB_IC_INFO.IC_CARDNO);
...
--DB2修改 调查问：MSC_QUESTION
ALTER TABLE MSC_QUESTION ALTER column REMARK SET DATA TYPE VARCHAR(260);

ALTER TABLE MSC_QUESTION_ITEM ALTER column QUESTION_OPTIONS SET DATA TYPE VARCHAR(960);
