...
select 1 as RELATION_FLAG,B.RELATION_ID,C.RELATI_ID as TENANT_ID,A.GODS_ID,
       ...
       coalesce(B.SHORT_GODS_NAME,A.SHORT_GODS_NAME) as SHORT_GODS_NAME,
...
from VIW_GOODSINFO A left join PUB_GOODSPRICE B ON A.TENANT_ID=B.TENANT_ID and A.GODS_ID=B.GODS_ID and ltrim(rtrim(char(A.TENANT_ID)))||'0001'=B.SHOP_ID and ltrim(rtrim(A.PRICE_ID))=ltrim(rtrim(B.PRICE_ID));
...
left join PUB_GOODSPRICE B ON C.SHOP_ID=B.SHOP_ID and A.TENANT_ID=B.TENANT_ID and A.GODS_ID=B.GODS_ID and ltrim(rtrim(A.PRICE_ID))=ltrim(rtrim(B.PRICE_ID));
