...
uses
  ...
  cxContainer, cxEdit, cxMaskEdit, cxDropDownEdit, Controls, DB, ZLogFile,
  ZConnection, Mask, ZAbstractRODataset, ZAbstractDataset, ZDataset;
...
type
  ...
  private
    ...
    procedure ExistDatabase;   // 2011.09.20 添加新函数
...
begin
  ...
  try
    ...
    if IsLocal then
      F.WriteString('localconnection',Key,Value)
    else
...
begin
  ...
  try
    F := TIniFile.Create(Path);
    if IsLocal then
      Result := F.ReadString('localconnection',Key,'')
    else
...
procedure TfrmDBSetup.ExistDatabase;
var Conn:TdbFactory;
    ConString,Str_DataBase:String;
    StrList:TStringList;
    i:Integer;
    rs:TZQuery;
begin
  ...
  try
    ...
    Conn.ConnMode := cbConnMode.ItemIndex + 1;
    StrList.Delimiter := ';';
    StrList.DelimitedText := ConString;
    StrList.Values['databasename'] := 'master';
    ConString := StrList.DelimitedText;
    Conn.Initialize(ConString);
    if not Conn.Connect then
      Raise Exception.Create('测试连接未通过,请检查各连接参数是否正确!');
    rs.Close;
    rs.SQL.Text := 'select * from sysdatabases where [name] = ''db_shop''';
    Conn.Open(rs);
    if rs.RecordCount < 1 then
    begin
      Str_DataBase := ' CREATE DATABASE db_shop ';
      try
        Conn.ExecSQL(Str_DataBase);
      except
        on E: Exception do
          begin
            LogFile.AddLogFile(0,Str_DataBase + ' 错误信息:' + E.Message);
            MessageBox(Application.MainForm.Handle,'db_shop 数据库创建失败！','友情提示...',MB_OK+MB_ICONINFORMATION);
          end;
      end;
    end;
  finally
