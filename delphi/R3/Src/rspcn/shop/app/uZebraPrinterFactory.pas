    ...
    begin
      ...
      while not rs.Eof do
        begin
          ...
          WindowsAztex(368, 3, 26, 4,'','',EncStr(LinkMan,ENC_KEY)+'|'+LicenseCode+'|'+BarCode+'|'+FormatFloat('#0.00',Aprice)+'|'+FormatDateTime('YYYYMMDDHHNNSSZZZ',printTime));//+'|'+EncStr(ShopName,ENC_KEY)+'|'+EncStr(Address,ENC_KEY)+'|'+EncStr(Telphone,ENC_KEY)+'|'+EncStr(GodsName,ENC_KEY));
