...
type
  ...
  TStockLocusNoAudit=class(TZProcFactory)
  public
    function Execute(AGlobal:IdbHelp;Params:TftParamList):Boolean;override;
  end;
  TStockLocusNoUnAudit=class(TZProcFactory)
  public
    function Execute(AGlobal:IdbHelp;Params:TftParamList):Boolean;override;
  end;
...
begin
  ...
  SelectSQL.Text :=
      ...
      ' j.REMARK,j.ORG_PRICE*j.AMOUNT as RTL_MONEY,h.TAX_RATE from STK_STOCKDATA j,STK_STOCKORDER h where j.TENANT_ID=h.TENANT_ID and j.STOCK_ID=h.STOCK_ID and j.TENANT_ID=:TENANT_ID and j.STOCK_ID=:STOCK_ID) jb inner join VIW_GOODSINFO b '+
...
begin
  ...
  begin
    SQL :=
      'UPDATE MKT_FITTINGDATA '+
      'SET '+
      '  OUT_AMT = ( '+
      '    SELECT '+
      '      sum(case when MKT_FITTINGDATA.UNIT_ID=c.CALC_UNITS then b.CALC_AMOUNT '+
                    ' when MKT_FITTINGDATA.UNIT_ID=c.SMALL_UNITS then cast((b.CALC_AMOUNT*1.000)/c.SMALLTO_CALC as decimal(18,3)) '+
                    ' when MKT_FITTINGDATA.UNIT_ID=c.BIG_UNITS then cast((b.CALC_AMOUNT*1.000)/c.BIGTO_CALC as decimal(18,3)) else b.CALC_AMOUNT end) '+
      '    FROM  '+
      '      STK_STOCKORDER a,'+
      '      STK_STOCKDATA b, '+
      '      VIW_GOODSINFO c '+
      ' WHERE a.TENANT_ID=b.TENANT_ID AND a.STOCK_ID=b.STOCK_ID and a.SHOP_ID=MKT_FITTINGDATA.SHOP_ID AND b.TENANT_ID=c.TENANT_ID AND b.GODS_ID=c.GODS_ID '+
      ' AND a.TENANT_ID=MKT_FITTINGDATA.TENANT_ID AND a.FIG_ID=MKT_FITTINGDATA.FIG_ID AND b.GODS_ID=MKT_FITTINGDATA.GODS_ID '+
      ' AND b.BATCH_NO=MKT_FITTINGDATA.BATCH_NO AND b.PROPERTY_01=MKT_FITTINGDATA.PROPERTY_01 '+
      ' AND b.PROPERTY_02=MKT_FITTINGDATA.PROPERTY_02 AND b.IS_PRESENT=MKT_FITTINGDATA.IS_PRESENT AND a.TENANT_ID=:TENANT_ID and a.FIG_ID=:FIG_ID '+
      ' ) '+
      'WHERE TENANT_ID=:TENANT_ID AND FIG_ID=:FIG_ID';
    AGlobal.ExecSQL(SQL,self);
  end;
end;
...
//         if roundto(FieldbyName('STOCK_MNY').AsFloat-FieldbyName('ADVA_MNY').AsFloat,-3)<>0 then
         begin
           ...
           + 'VALUES(:ABLE_ID,:TENANT_ID,:SHOP_ID,:DEPT_ID,:CLIENT_ID,'''+'进货货款【入库单号'+FieldbyName('GLIDE_NO').AsString+'】'+''',''4'',:STOCK_MNY,0,:ADVA_MNY,:RECK_MNY,:STOCK_DATE,:STOCK_ID,:CREA_DATE,:CREA_USER,''00'','+GetTimeStamp(iDbType)+')';
...
begin
  ...
  1:SelectSQL.Text := 'select * from (select STOCK_ID from STK_STOCKORDER where SHOP_ID=:SHOP_ID and TENANT_ID=:TENANT_ID and CREA_USER=:CREA_USER and GLIDE_NO>:GLIDE_NO and STOCK_TYPE=:STOCK_TYPE order by GLIDE_NO) where ROWNUM=1';
...
begin
  ...
  try
    rs.SQL.Text :=
      'select LOCUS_STATUS from STK_STOCKORDER A where TENANT_ID='+Params.FindParam('TENANT_ID').asString+' and STOCK_ID='''+Params.FindParam('STOCK_ID').asString+''' '+
      ' and Exists(select * from STK_STOCKDATA B,VIW_GOODSINFO C where B.TENANT_ID=C.TENANT_ID and B.GODS_ID=C.GODS_ID and C.USING_LOCUS_NO=1 and B.TENANT_ID=A.TENANT_ID and B.STOCK_ID=A.STOCK_ID)';
    AGlobal.Open(rs);
...
begin
  ...
  SelectSQL.Text :=
               ...
               'select jb.*,b.CLIENT_NAME as CLIENT_ID_TEXT from '+
               '(select TENANT_ID,SHOP_ID,DEPT_ID,STOCK_ID,GLIDE_NO,STOCK_TYPE,STOCK_DATE,GUIDE_USER,CLIENT_ID,CHK_DATE,CHK_USER,FROM_ID,FIG_ID,INVOICE_FLAG,'+
               'STOCK_MNY,STOCK_AMT,ADVA_MNY,TAX_RATE,REMARK,COMM,CREA_DATE,CREA_USER,TIME_STAMP,COMM_ID,'+
               'LOCUS_STATUS,LOCUS_USER,LOCUS_DATE,LOCUS_AMT,LOCUS_CHK_DATE,LOCUS_CHK_USER from STK_STOCKORDER where TENANT_ID=:TENANT_ID and STOCK_ID=:STOCK_ID) jb '+
               ' left outer join VIW_CLIENTINFO b on jb.TENANT_ID=b.TENANT_ID and jb.CLIENT_ID=b.CLIENT_ID ) jc '+
               ' left outer join VIW_USERS c on jc.TENANT_ID=c.TENANT_ID and jc.GUIDE_USER=c.USER_ID ) jd '+
               ' left outer join VIW_USERS d on jd.TENANT_ID=d.TENANT_ID and jd.CHK_USER=d.USER_ID ) je '+
               ' left outer join CA_SHOP_INFO e on  je.TENANT_ID=e.TENANT_ID and je.SHOP_ID=e.SHOP_ID ) jf '+
...
function TStockLocusNoAudit.Execute(AGlobal: IdbHelp;
  Params: TftParamList): Boolean;
var
  Str:string;
  n:Integer;
...
{ TStockLocusNoUnAudit }

function TStockLocusNoUnAudit.Execute(AGlobal: IdbHelp;
  Params: TftParamList): Boolean;
var Str:string;
    n:Integer;
begin
...
initialization
  ...
  RegisterClass(TStockLocusNoAudit);
  RegisterClass(TStockLocusNoUnAudit);
finalization
  ...
  UnRegisterClass(TStockLocusNoAudit);
  UnRegisterClass(TStockLocusNoUnAudit);
end.
