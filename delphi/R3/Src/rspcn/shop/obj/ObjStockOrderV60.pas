...
procedure TStockDataV60.CreateNew(AOwner: TComponent);
begin
  inherited;
  Updates := TUpdateStroageList.Create;
end;

destructor TStockDataV60.Destroy;
begin
  Updates.Free;
  inherited;
end;
...
begin
  ...
  SelectSQL.Text :=
      ...
      ' j.REMARK,j.ORG_PRICE*j.AMOUNT as RTL_MONEY,j.TAX_RATE from STK_STOCKDATA j,STK_STOCKORDER h where j.TENANT_ID=h.TENANT_ID and j.STOCK_ID=h.STOCK_ID and j.TENANT_ID=:TENANT_ID and j.STOCK_ID=:STOCK_ID) jb inner join ('+Params.ParambyName('VIW_GOODSINFO').AsString+') b '+
      ' on jb.TENANT_ID=b.TENANT_ID and jb.GODS_ID=b.GODS_ID order by jb.SEQNO';
  IsSQLUpdate := True;
  Str := ' insert into STK_STOCKDATA(TENANT_ID,SHOP_ID,SEQNO,STOCK_ID,GODS_ID,BATCH_NO,LOCUS_NO,BOM_ID,PROPERTY_01,PROPERTY_02,UNIT_ID,AMOUNT,ORG_PRICE,IS_PRESENT,APRICE,AMONEY,AGIO_RATE,AGIO_MONEY,CALC_AMOUNT,CALC_MONEY,REMARK,TAX_RATE) '+
         ' values (:TENANT_ID,:SHOP_ID,:SEQNO,:STOCK_ID,:GODS_ID,:BATCH_NO,:LOCUS_NO,:BOM_ID,:PROPERTY_01,:PROPERTY_02,:UNIT_ID,:AMOUNT,:ORG_PRICE,:IS_PRESENT,:APRICE,:AMONEY,:AGIO_RATE,:AGIO_MONEY,:CALC_AMOUNT,:CALC_MONEY,:REMARK,:TAX_RATE)';
  InsertSQL.Text := Str;
  Str := ' update STK_STOCKDATA set TENANT_ID=:TENANT_ID,SHOP_ID=:SHOP_ID,SEQNO=:SEQNO,STOCK_ID=:STOCK_ID,GODS_ID=:GODS_ID,BATCH_NO=:BATCH_NO,LOCUS_NO=:LOCUS_NO,BOM_ID=:BOM_ID,'+
         ...
         ' IS_PRESENT=:IS_PRESENT,APRICE=:APRICE,AMONEY=:AMONEY,AGIO_RATE=:AGIO_RATE,AGIO_MONEY=:AGIO_MONEY,CALC_AMOUNT=:CALC_AMOUNT,CALC_MONEY=:CALC_MONEY,REMARK=:REMARK,TAX_RATE=:TAX_RATE '+
...
function TSyncStockDataV60.BeforeCommitRecord(AGlobal: IdbHelp): Boolean;
begin
  Updates.DoUpdate(AGlobal);
end;

procedure TSyncStockDataV60.CreateNew(AOwner: TComponent);
begin
  inherited;
  Updates := TUpdateStroageList.Create;
end;

destructor TSyncStockDataV60.Destroy;
begin
  Updates.Free;
  inherited;
end;
