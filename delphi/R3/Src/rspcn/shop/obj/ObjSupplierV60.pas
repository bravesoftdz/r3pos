...
function TSupplierV60.BeforeDeleteRecord(AGlobal: IdbHelp): Boolean;
var Str: String;
    rs:TZQuery;
begin
  Result := False;
  ...
  try
  if FieldByName('CLIENT_TYPE').AsString='2' then
  begin
  ...
  end else if FieldByName('CLIENT_TYPE').AsString='1' then
  begin
...
function TSupplierV60.BeforeInsertRecord(AGlobal: IdbHelp): Boolean;
var Temp: TZQuery;
    Str: String;
begin
  if FieldByName('CLIENT_TYPE').AsString='2' then
...
function TSupplierV60.BeforeModifyRecord(AGlobal: IdbHelp): Boolean;
var Temp: TZQuery;
    Str: String;
    n:Integer;
begin
...
procedure TSupplierV60.InitClass;
var Str: string;
begin
  inherited;
  ...
  'QQ,MSN,BANK_ID,ACCOUNT,INVOICE_FLAG,REMARK,TAX_RATE,PRICE_ID,SHOP_ID,ORGAN_NATURE,ORGAN_GRADE,DECISION_POST,CIGARETTE_PRICE,COLLECT_MODE,'+
  'TENANT_SCALE,PUR_FREQUENCY,DECISION_FAV_BRAND,ASSOCIATE_STAFF from PUB_CLIENTINFO '+
  ...
  'SHOP_ID,ORGAN_NATURE,ORGAN_GRADE,DECISION_POST,CIGARETTE_PRICE,COLLECT_MODE,TENANT_SCALE,PUR_FREQUENCY,DECISION_FAV_BRAND,ASSOCIATE_STAFF,'+
  'COMM,TIME_STAMP) values(:TENANT_ID,:CLIENT_ID,:CLIENT_TYPE,:CLIENT_CODE,:LICENSE_CODE,:CLIENT_NAME,:CLIENT_SPELL,:SORT_ID,:REGION_ID,'+
  ...
  ':RECV_TELE,:LEGAL_REPR,:INVO_NAME,:TAX_NO,:FAXES,:HOMEPAGE,:EMAIL,:QQ,:MSN,:BANK_ID,:ACCOUNT,:INVOICE_FLAG,:REMARK,:TAX_RATE,:PRICE_ID,:SHOP_ID,'+
  ':ORGAN_NATURE,:ORGAN_GRADE,:DECISION_POST,:CIGARETTE_PRICE,:COLLECT_MODE,:TENANT_SCALE,:PUR_FREQUENCY,:DECISION_FAV_BRAND,:ASSOCIATE_STAFF,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Text := Str;
  ...
  'INVOICE_FLAG=:INVOICE_FLAG,REMARK=:REMARK,TAX_RATE=:TAX_RATE,PRICE_ID=:PRICE_ID,SHOP_ID=:SHOP_ID,ORGAN_NATURE=:ORGAN_NATURE,ORGAN_GRADE=:ORGAN_GRADE,DECISION_POST=:DECISION_POST,'+
  'CIGARETTE_PRICE=:CIGARETTE_PRICE,COLLECT_MODE=:COLLECT_MODE,TENANT_SCALE=:TENANT_SCALE,PUR_FREQUENCY=:PUR_FREQUENCY,DECISION_FAV_BRAND=:DECISION_FAV_BRAND,ASSOCIATE_STAFF=:ASSOCIATE_STAFF,'+
...
initialization
  RegisterClass(TSupplierV60);
finalization
  UnRegisterClass(TSupplierV60);
end.
