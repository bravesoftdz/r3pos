...
begin
  ...
  SelectSQL.Text := 'select TENANT_ID,LICENSE_CODE,LOGIN_NAME,TENANT_NAME,TENANT_TYPE,SHORT_TENANT_NAME,TENANT_SPELL,LEGAL_REPR,LINKMAN,TELEPHONE,FAXES,'+
                    'HOMEPAGE,QQ,MSN,ADDRESS,POSTALCODE,REMARK,PASSWRD,REGION_ID,SRVR_ID,PROD_ID,CREA_DATE,AUDIT_STATUS from CA_TENANT where TENANT_ID=:TENANT_ID';
  IsSQLUpdate := true;
  ...
  Str := 'insert into CA_TENANT (TENANT_ID,LICENSE_CODE,LOGIN_NAME,TENANT_NAME,TENANT_TYPE,SHORT_TENANT_NAME,TENANT_SPELL,LEGAL_REPR,LINKMAN,TELEPHONE,FAXES,'+
         'HOMEPAGE,QQ,MSN,ADDRESS,POSTALCODE,REMARK,PASSWRD,REGION_ID,SRVR_ID,PROD_ID,CREA_DATE,AUDIT_STATUS,COMM,TIME_STAMP) VALUES (:TENANT_ID,:LICENSE_CODE,:LOGIN_NAME,:TENANT_NAME,:TENANT_TYPE,:SHORT_TENANT_NAME,'+
         ':TENANT_SPELL,:LEGAL_REPR,:LINKMAN,:TELEPHONE,:FAXES,:HOMEPAGE,:QQ,:MSN,:ADDRESS,:POSTALCODE,:REMARK,:PASSWRD,:REGION_ID,:SRVR_ID,:PROD_ID,:CREA_DATE,:AUDIT_STATUS,''00'','+GetTimeStamp(iDbType)+')';
  InsertSQL.Add(Str);
  ...
  Str := 'update CA_TENANT set LOGIN_NAME=:LOGIN_NAME,TENANT_NAME=:TENANT_NAME,LICENSE_CODE=:LICENSE_CODE,TENANT_TYPE=:TENANT_TYPE,SHORT_TENANT_NAME=:SHORT_TENANT_NAME,'+
         ...
         'REMARK=:REMARK,QQ=:QQ,MSN=:MSN,PASSWRD=:PASSWRD,REGION_ID=:REGION_ID,SRVR_ID=:SRVR_ID,CREA_DATE=:CREA_DATE,AUDIT_STATUS=:AUDIT_STATUS,TIME_STAMP='+GetTimeStamp(iDbType)+' where TENANT_ID=:OLD_TENANT_ID';
  UpdateSQL.Add(Str);
end;
...
begin
  ...
  rs := TZQuery.Create(nil);
  try
    rs.SQL.Text := 'select count(*) from PUB_PRICEGRADE where TENANT_ID='+Params.ParambyName('TENANT_ID').AsString+' and PRICE_ID=''332E5CE8-D1FC-4D53-84C3-2A1EB07EF8AA'' ';
    AGlobal.Open(rs);
    if rs.Fields[0].AsInteger > 0 then
       begin
         result := true;
...
begin
  ...
  try
    ...
    begin
      ...
      'values(:TENANT_ID,''#'',''普通客户'',''PT'',0,0,0,0,0,0,1,''00'',5497000,''1'')';
      AGlobal.ExecSQL(Str,Params);
      Str := 'insert into PUB_PRICEGRADE(TENANT_ID,PRICE_ID,PRICE_NAME,PRICE_SPELL,INTEGRAL,INTE_TYPE,INTE_AMOUNT,MINIMUM_PERCENT,AGIO_TYPE,AGIO_PERCENT,SEQ_NO,COMM,TIME_STAMP,PRICE_TYPE)'+
      'values(:TENANT_ID,''332E5CE8-D1FC-4D53-84C3-2A1EB07EF8AA'',''银卡会员'',''YK'',0,0,0,0,0,0,1,''00'',5497000,''1'')';
      AGlobal.ExecSQL(Str,Params);
      Str := 'insert into PUB_PRICEGRADE(TENANT_ID,PRICE_ID,PRICE_NAME,PRICE_SPELL,INTEGRAL,INTE_TYPE,INTE_AMOUNT,MINIMUM_PERCENT,AGIO_TYPE,AGIO_PERCENT,SEQ_NO,COMM,TIME_STAMP,PRICE_TYPE)'+
      'values(:TENANT_ID,''26AB5597-5AA5-49BC-8CBF-7A332CB1B6FA'',''金卡会员'',''JK'',0,0,0,0,0,0,1,''00'',5497000,''1'')';
      AGlobal.ExecSQL(Str,Params);
      Str := 'insert into PUB_PRICEGRADE(TENANT_ID,PRICE_ID,PRICE_NAME,PRICE_SPELL,INTEGRAL,INTE_TYPE,INTE_AMOUNT,MINIMUM_PERCENT,AGIO_TYPE,AGIO_PERCENT,SEQ_NO,COMM,TIME_STAMP,PRICE_TYPE)'+
      'values(:TENANT_ID,''4433497A-0A2E-4D72-BE0B-723850A6EFE8'',''钻石会员'',''ZS'',0,0,0,0,0,0,1,''00'',5497000,''1'')';
      AGlobal.ExecSQL(Str,Params);
    end;
    result := true;
    AGlobal.CommitTrans;
