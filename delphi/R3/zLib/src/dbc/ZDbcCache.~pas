...
begin
  ...
  begin
    ...
    case FColumnTypes[ColumnIndex] of
      ...
      stString:
        begin
          //zhangsenrong 2011-08-15 
          if FColumnLengths[ColumnIndex]=256 then
             begin
               if IsNumber(StrPas(PChar(ValuePtr1))) and IsNumber(StrPas(PChar(ValuePtr2))) then
                  begin
                    Result := CompareFloat(StrtoFloatDef(StrPas(PChar(ValuePtr1)),0),StrtoFloatDef(StrPas(PChar(ValuePtr2)),0));
                  end
               else
